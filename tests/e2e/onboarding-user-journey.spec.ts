...
test.describe('Onboarding and Complete User Journey', () => {
  ...
  test('should complete full onboarding flow with Guardian of Memories narrative', async ({ page }) => {
    // Start at the onboarding page
    await page.goto('/onboarding');

    // Scene 1 - Promise of Calm
    await expect(page.locator('[data-testid="onboarding-scene-1"]')).toBeVisible();
    await expect(page.locator('[data-testid="firefly-animation"]')).toBeVisible();
    await expect(page.locator('text=Guardian of Memories')).toBeVisible();

    // Check for night scene background
    await expect(page.locator('[data-testid="night-scene-background"]')).toBeVisible();

    // Click "Start writing my story" CTA
    await page.click('[data-testid="start-story-button"]');

    // Scene 2 - The Box of Certainty
    await expect(page.locator('[data-testid="onboarding-scene-2"]')).toBeVisible();
    await expect(page.locator('[data-testid="box-of-certainty"]')).toBeVisible();
    await expect(page.locator('text=Box of Certainty')).toBeVisible();

    // Should show emotional prompt about items for loved ones
    await expect(page.locator('[data-testid="emotional-prompt"]')).toBeVisible();
    ...
    await expect(page.locator('[data-testid="onboarding-scene-3"]')).toBeVisible();
    ...
    await expect(page.locator('[data-testid="name-entry-field"]')).toBeVisible();
    ...
    await expect(page.locator('[data-testid="key-engraving"]')).toContainText('For John Doe');

    // Continue to final scene
    await page.click('[data-testid="continue-to-scene-4"]');

    // Scene 4 - Preparing the Path
    await expect(page.locator('[data-testid="onboarding-scene-4"]')).toBeVisible();
    await expect(page.locator('[data-testid="preparing-path"]')).toBeVisible();

    // Should show firefly light trail animation
    await expect(page.locator('[data-testid="firefly-trail-animation"]')).toBeVisible();
  ...
  test('should handle onboarding with personality mode selection', async ({ page }) => {
    await page.goto('/onboarding');
    ...
    const personalitySelector = page.locator('[data-testid="personality-mode-selector"]');
  ...
  test('should show onboarding progress indicators', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="progress-indicator"]')).toContainText('1 / 4');
  ...
  test('should complete first document upload user journey', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="empty-vault-message"]')).toBeVisible();
    ...
    if (await fileInput.isVisible()) {
      ...
      await page.fill('[data-testid="document-description"]', 'Important family document');
    ...
    const milestoneMessage = page.locator('[data-testid="first-stone-milestone"]');
    if (await milestoneMessage.isVisible()) {
      await expect(milestoneMessage).toContainText("first stone in your family's mosaic");
    }
  });
  ...
  test('should show pillar unlock progression system', async ({ page }) => {
    ...
    const todayPillarState = await todayPillar.getAttribute('data-pillar-state');
    ...
    const tomorrowPillarState = await tomorrowPillar.getAttribute('data-pillar-state');
  ...
  test('should show document expiry notification workflow', async ({ page }) => {
    await page.goto('/dashboard');

    // Check for document expiry notifications
    const expiryNotification = page.locator('[data-testid="document-expiry-notification"]');
    if (await expiryNotification.isVisible()) {
      await expect(expiryNotification).toBeVisible();

      // Should show friendly reminder message
      await expect(expiryNotification).toContainText("small reminder from your memory guardian");

      // Should have action buttons
      await expect(page.locator('[data-testid="update-document-button"]')).toBeVisible();
      await expect(page.locator('[data-testid="dismiss-notification-button"]')).toBeVisible();
  ...
  test('should complete guardian setup user journey', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="guardian-setup-form"]')).toBeVisible();
    ...
    await expect(page.locator('[data-testid="guardian-list"]')).toContainText('Sarah Johnson');
  });
  ...
  test('should show legacy planning progression workflow', async ({ page }) => {
    ...
    const recommendations = page.locator('[data-testid="legacy-recommendations"]');
  ...
  test('should complete Time Capsule premium feature workflow', async ({ page }) => {
    await page.goto('/legacy');

    // Look for Time Capsule feature
    const timeCapsuleSection = page.locator('[data-testid="time-capsule-section"]');
    if (await timeCapsuleSection.isVisible()) {
      await expect(timeCapsuleSection).toBeVisible();

      // Should show video message creation
      await expect(page.locator('[data-testid="create-video-message"]')).toBeVisible();

      // Click create video message
      await page.click('[data-testid="create-video-message"]');

      // Should show video message form
      await expect(page.locator('[data-testid="video-message-form"]')).toBeVisible();

      // Fill message details
      await page.fill('[data-testid="message-recipient"]', 'Anna');
      await page.selectOption('[data-testid="delivery-occasion"]', '18th birthday');
      await page.fill('[data-testid="message-preview"]', 'Happy 18th birthday, Anna!');

      // Should show delivery date setting
      await expect(page.locator('[data-testid="delivery-date-setting"]')).toBeVisible();
    }
  });
  ...
  test('should show annual ritual and progress summary', async ({ page }) => {
    ...
    const annualRitual = page.locator('[data-testid="annual-ritual-notification"]');
    if (await annualRitual.isVisible()) {
      await expect(annualRitual).toBeVisible();

      // Should show progress summary
      await expect(page.locator('[data-testid="annual-progress-summary"]')).toBeVisible();

      // Should show reflective nudge
      const reflectiveMessage = page.locator('[data-testid="reflective-message"]');
      await expect(reflectiveMessage).toBeVisible();

      // Check for year-over-year progress
      const progressComparison = page.locator('[data-testid="progress-comparison"]');
  ...
  test('should show Box of Certainty completion celebration', async ({ page }) => {
    await page.goto('/legacy');

    // Check for Box of Certainty completion state
    const boxCompletion = page.locator('[data-testid="box-of-certainty-completion"]');
    if (await boxCompletion.isVisible()) {
      // Should show glow animation when pillars are ~90%+ complete
      await expect(page.locator('[data-testid="box-glow-animation"]')).toBeVisible();
  ...
  test('should handle mobile responsive onboarding flow', async ({ page }) => {
    ...
    // Check mobile layout
    await expect(page.locator('[data-testid="onboarding-mobile-layout"]')).toBeVisible();
    ...
    if (await firefly.isVisible()) {
      ...
      expect(parseFloat(animationConfig.animationDuration)).toBeLessThanOrEqual(0.5);
    }
    ...
    await page.waitForURL('**/dashboard');
    await expect(page.locator('[data-testid="dashboard-mobile-layout"]')).toBeVisible();
  });
  ...
  test('should handle onboarding accessibility features', async ({ page }) => {
    ...
    await expect(focusedElement).toHaveAttribute('data-testid', 'start-story-button');

    // Check for ARIA labels
    await expect(page.locator('[data-testid="onboarding-scene-1"]')).toHaveAttribute('role', 'main');
    await expect(page.locator('[data-testid="progress-indicator"]')).toHaveAttribute('aria-label');
