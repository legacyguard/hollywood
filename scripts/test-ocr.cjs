...
async function testOCRSetup() {
  ...
  try {
    ...
    console.log('  ⚠️  Note: For full testing, upload an actual document image');
    ...
    if (error.code === 7) {
      ...
      console.log('  1. The Cloud Vision API is enabled in your Google Cloud project');
  ...
  console.log(`  🔐 Private Key ID: ${process.env.GOOGLE_CLOUD_PRIVATE_KEY_ID}`);
  ...
  console.log('  📊 Monitor usage: https://console.cloud.google.com/apis/api/vision.googleapis.com');
