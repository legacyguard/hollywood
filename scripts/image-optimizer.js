...
class ImageOptimizer {
  ...
  async processImage(imageInfo) {
    try {
      ...
      console.log(`   ❌ Error processing ${imageInfo.relativePath}: ${error.message}`);
      this.stats.errors++;
    }
  }
  ...
  async generateWebP(image, imageInfo, metadata) {
    try {
      ...
      const webpImage = image.clone().webp({ quality: this.config.quality.webp });
  ...
  async generateAVIF(image, imageInfo, metadata) {
    try {
      ...
      const avifImage = image.clone().avif({ quality: this.config.quality.avif });
  ...
  async optimizeOriginal(image, imageInfo, metadata) {
    try {
      ...
      // Resize if too large
      if (metadata.width > this.config.maxWidth || metadata.height > this.config.maxHeight) {
        optimizedImage = optimizedImage.resize(this.config.maxWidth, this.config.maxHeight, {
          fit: 'inside',
          withoutEnlargement: true
        });
      }
      ...
      if (ext === '.png') {
        optimizedImage = optimizedImage.png({ quality: 90, compressionLevel: 9 });
      } else if (['.jpg', '.jpeg'].includes(ext)) {
        optimizedImage = optimizedImage.jpeg({ quality: 85, progressive: true });
      }
  ...
  generateRecommendations() {
    ...
    if (this.stats.webpGenerated > 0) {
      recommendations.push('Use WebP format for better compression and faster loading');
    }
    ...
    if (this.stats.optimized > 0) {
      recommendations.push('Original images have been optimized for better performance');
    }
  ...
  async updatePackageScripts() {
    try {
      ...
      packageJson.scripts['optimize:performance'] = 'node scripts/performance-automation.js';
      packageJson.scripts['build:optimized'] = 'npm run optimize:images && npm run build';
