...
const BUNDLE_ANALYSIS_OUTPUT = path.join(__dirname, '..', 'bundle-analysis.json');
...
function analyzeJSChunks() {
  ...
  return jsFiles.map(file => {
    ...
    return {
      ...
      category,
      sizeKB,
      status: sizeKB > THRESHOLDS.CHUNK_ERROR ? 'error' : 
              sizeKB > THRESHOLDS.CHUNK_WARNING ? 'warning' : 'ok'
    };
...
function generateRecommendations(jsChunks, cssAssets) {
  ...
  const largeChunks = jsChunks.filter(chunk => chunk.sizeKB > THRESHOLDS.CHUNK_WARNING);
  ...
  const vendorChunks = jsChunks.filter(chunk => chunk.category.includes('vendor'));
  ...
  if (featureChunks.length > 5) {
    recommendations.push({
      ...
      action: 'Consider consolidating related features or implementing lazy loading'
    });
  }
...
function analyzeBundlePerformance() {
  ...
  const analysis = {
    ...
    summary: {
      ...
      status: totalBundleSize > THRESHOLDS.TOTAL_ERROR ? 'error' :
              totalBundleSize > THRESHOLDS.TOTAL_WARNING ? 'warning' : 'good'
    },
    chunks: jsChunks,
    css: cssAssets,
  ...
  // Display status
  const statusIcon = analysis.summary.status === 'good' ? '✅' : 
                    analysis.summary.status === 'warning' ? '⚠️' : '❌';
  console.log(`${statusIcon} Bundle Status: ${analysis.summary.status.toUpperCase()}\n`);
  
  // Show top 5 largest chunks
  console.log('🏆 Top 5 Largest Chunks:');
  jsChunks.slice(0, 5).forEach((chunk, index) => {
    const statusIcon = chunk.status === 'ok' ? '✅' : chunk.status === 'warning' ? '⚠️' : '❌';
    console.log(`   ${index + 1}. ${chunk.chunkName} (${chunk.category}) - ${chunk.sizeKB} KB ${statusIcon}`);
  });
  console.log();
