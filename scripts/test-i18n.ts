...
class I18nTester {
  ...
  private testFilePathMapping() {
    ...
    testCases.forEach(test => {
      ...
      if (parts[0] === 'common' || parts[0] === 'features' || parts[0] === 'auth' || parts[0] === 'errors') {
      ...
      this.results.push({
        ...
        error: computed !== test.expectedPath ? 
          `Expected: ${test.expectedPath}, Got: ${computed}` : undefined
      });
    });
  }

  private testNamespaceConfiguration() {
    const configPath = path.join(process.cwd(), 'locales/config/languages.json');
    ...
    try {
      ...
      const requiredFields = ['supportedLanguages', 'defaultLanguage', 'namespaces'];
      ...
      this.results.push({
        ...
        error: missingFields.length > 0 ? 
          `Missing fields: ${missingFields.join(', ')}` : undefined
      });
    } catch (error) {
      this.results.push({
        name: 'Namespace configuration',
  ...
  private testDuplicateKeys() {
    const enPath = path.join(process.cwd(), 'locales/_shared/en/common/ui.json');
    const skPath = path.join(process.cwd(), 'locales/_shared/sk/common/ui.json');
  ...
  private extractKeys(obj: any, prefix = ''): string[] {
    ...
    for (const key in obj) {
      ...
      if (typeof obj[key] === 'object' && obj[key] !== null && !key.includes('_')) {
  ...
  private printResults() {
    ...
    console.log(`${colors.green}Passed: ${passed}${colors.reset} | `, end='');
