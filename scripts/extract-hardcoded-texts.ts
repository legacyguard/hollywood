...
class TextExtractor {
  ...
  private isUserFacing(path: any): boolean {
    ...
    if (t.isJSXAttribute(parent)) {
      ...
      const userFacingAttrs = [
        ...
        'aria-description', 'helperText', 'errorMessage', 'description',
    ...
    if (t.isCallExpression(parent)) {
      ...
      if (t.isIdentifier(callee)) {
        const userFacingFunctions = [
          'alert', 'confirm', 'prompt', 'toast', 'notify',
          'setError', 'setMessage', 'console.log', 'console.error'
        ];
    ...
    if (t.isObjectProperty(parent)) {
      ...
      if (t.isIdentifier(key)) {
        const userFacingProps = [
          'title', 'description', 'message', 'label', 'text',
          'error', 'success', 'warning', 'info', 'placeholder'
        ];
  ...
  private getContext(path: any): string {
    ...
    const functionParent = path.findParent((p: any) => 
      t.isFunctionDeclaration(p) || t.isArrowFunctionExpression(p)
    );
  ...
  private addText(text: HardcodedText): void {
    ...
    const exists = this.texts.some(t => 
      t.text === text.text && 
      t.file === text.file &&
      t.line === text.line
    );
  ...
  private suggestNamespace(text: HardcodedText): string {
    ...
    if (filePath.includes('/auth/') || filePath.includes('login') || filePath.includes('register')) {
      return 'auth';
    }
  ...
  exportReport(report: ExtractionReport, format: 'json' | 'csv' | 'markdown' = 'json'): void {
  ...
  private exportCSV(report: ExtractionReport, outputDir: string): void {
    const csvLines = ['File,Line,Column,Text,Type,Context,Component,Suggestion'];
  ...
  private exportMarkdown(report: ExtractionReport, outputDir: string): void {
    ...
    for (const [file, texts] of report.byFile) {
      ...
      for (const text of texts) {
        lines.push(`| ${text.line} | ${text.text} | ${text.type} | ${text.suggestion || '-'} |`);
      }
      lines.push('');
    }
