...
function fixJSXObjectAttributes(content, filePath) {
  ...
  const brokenObjectPatterns = [
    ...
    {
      pattern:
        /(initial|animate|exit|transition|whileHover|whileTap|style|viewport|custom|drag|layout|layoutId)=\s*{\s*([a-zA-Z_$][a-zA-Z0-9_$]*\s*:)/g,
      replacement: '$1={{ $2',
    },
    // Fix closing braces for these attributes
    {
      pattern:
        /(initial|animate|exit|transition|whileHover|whileTap|style|viewport|custom|drag|layout|layoutId)={{([^}]+)}(?!})/g,
      replacement: '$1={{ $2 }}',
    },
  ...
  if (fixCount > 0) {
    console.log(
      `${colors.green}Fixed ${fixCount} JSX object attribute issues in ${path.basename(filePath)}${colors.reset}`
    );
  }
...
function fixDestructuringSyntax(content, filePath) {
  ...
  const destructuringPattern =
    /const\s+\[([a-zA-Z_$][a-zA-Z0-9_$]*),\s*([a-zA-Z_$][a-zA-Z0-9_$]*)\s*=\s*(useState|useReducer|useMemo|useCallback)/g;
  ...
  if (fixCount > 0) {
    console.log(
      `${colors.green}Fixed ${fixCount} destructuring syntax issues in ${path.basename(filePath)}${colors.reset}`
    );
  }
...
// Main execution
console.log(
  `${colors.bright}${colors.blue}Starting JSX Syntax Fix Script${colors.reset}`
);
console.log(
  `${colors.cyan}Scanning for TypeScript/React files...${colors.reset}\n`
);
...
if (filesFixed === 0) {
  console.log(
    `${colors.yellow}No JSX syntax issues found that match the patterns.${colors.reset}`
  );
}
