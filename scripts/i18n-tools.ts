...
class TranslationOrganizer {
  ...
  async analyzeCurrentStructure(): Promise<TranslationReport> {
    ...
    for (const file of files) {
      ...
      (report.languages as any).add(language);
      (report.namespaces as any).add(namespace);
    ...
    report.languages = Array.from(report.languages as any);
    report.namespaces = Array.from(report.namespaces as any);
  ...
  private checkConsistency(report: TranslationReport) {
    ...
    for (const [, module] of this.modules) {
      ...
      namespaceGroups.get(module.namespace)!.push(module);
    }
