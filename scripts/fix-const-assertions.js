...
function fixConstAssertions(content, filePath) {
  ...
  const unionTypePattern = /:\s*(['"])([^'"]+)\1\s+as\s+const\s*\|\s*(['"])([^'"]+)\3/g;
  ...
  if (fixCount > 0) {
    console.log(`${colors.green}Fixed ${fixCount} const assertion issues in ${path.basename(filePath)}${colors.reset}`);
  }
...
function fixOtherSyntaxErrors(content, filePath) {
  ...
  if (matches) {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (emptyAccessPattern.test(lines[i]) && !lines[i].includes('[]') && !lines[i].trim().startsWith('//')) {
  ...
  if (fixCount > 0) {
    console.log(`${colors.yellow}Commented out ${fixCount} lines with empty array access in ${path.basename(filePath)}${colors.reset}`);
  }
...
// Main execution
console.log(`${colors.bright}${colors.blue}Starting Const Assertion Fix Script${colors.reset}`);
...
if (filesFixed === 0) {
  console.log(`${colors.yellow}No const assertion issues found that match the patterns.${colors.reset}`);
}
