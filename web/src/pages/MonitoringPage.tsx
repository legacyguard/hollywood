...
export default function MonitoringPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default:
        return <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200">Unhealthy</Badge>;
    }
  };
                ...
                {Object.entries(healthData.checks).map(([name, check]: [string, any]) => (
                  <div key={name} className="flex items-center justify-between p-4 border rounded-lg">
                    ...
                    <div className="text-right">
                      <div className="text-sm font-medium">{check.duration}ms</div>
                ...
                <div className="space-y-3">
                  {Object.entries(performanceData.webVitals).map(([key, metric]: [string, any]) => (
                    <div key={key} className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-2">
                        <span className="text-sm">{Math.round(metric.value)}ms</span>
              ...
              {performanceData?.memory ? (
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{formatBytes(performanceData.memory.usedJSHeapSize)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Total Heap</span>
                    <span className="font-medium">{formatBytes(performanceData.memory.totalJSHeapSize)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Heap Limit</span>
                    <span className="font-medium">{formatBytes(performanceData.memory.jsHeapSizeLimit)}</span>
                  </div>
                  <Separator />
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Usage</span>
                    <span className={`font-medium ${
                      (performanceData.memory.usedJSHeapSize / performanceData.memory.jsHeapSizeLimit) > 0.8 ? 'text-red-600' :
                      (performanceData.memory.usedJSHeapSize / performanceData.memory.jsHeapSizeLimit) > 0.6 ? 'text-yellow-600' :
                      'text-green-600'
                    }`}>
                      {Math.round((performanceData.memory.usedJSHeapSize / performanceData.memory.jsHeapSizeLimit) * 100)}%
                    </span>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  ...
                  <div className="font-medium">{performanceData.connection.effectiveType || 'Unknown'}</div>
                </div>
                <div>
                  <div className="text-sm text-muted-foreground">Downlink</div>
                  <div className="font-medium">{performanceData.connection.downlink} Mbps</div>
                </div>
                <div>
                  <div className="text-sm text-muted-foreground">RTT</div>
                  <div className="font-medium">{performanceData.connection.rtt}ms</div>
                </div>
                <div>
                  <div className="text-sm text-muted-foreground">Save Data</div>
                  <div className="font-medium">{performanceData.connection.saveData ? 'On' : 'Off'}</div>
                </div>
              </div>
            </CardContent>
          </Card>
