...
export function FamilyMemberDashboard({ userId }: FamilyMemberDashboardProps) {
  ...
  const filteredMembers = members.filter(member => {
    switch (filter) {
      ...
      case 'pending': return member.status === 'invited' || member.status === 'pending_verification';
      case 'emergency': return member.role === 'emergency_contact';
      default: return true;
    }
  });
      ...
      {protectionStatus && (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{protectionStatus.totalMembers}</div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{protectionStatus.protectionLevel}%</div>
              <Progress value={protectionStatus.protectionLevel} className="mt-2" />
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{protectionStatus.documentsShared}</div>
              <p className="text-xs text-muted-foreground">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Manage your family circle and their access to important documents
              </CardDescription>
            </div>
        ...
        <CardContent>
          ...
          <div className="flex gap-2 mb-6">
            {[
              ...
              { key: 'pending', label: 'Pending', count: members.filter(m => m.status === 'invited' || m.status === 'pending_verification').length },
              { key: 'emergency', label: 'Emergency', count: members.filter(m => m.role === 'emergency_contact').length }
            ].map(({ key, label, count }) => (
              <Button
                key={key}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem
                                onClick={() => handleMemberAction({ type: 'edit', member })}
                              >
                                Edit Member
                              </DropdownMenuItem>
                              <DropdownMenuItem
                                onClick={() => handleMemberAction({ type: 'view_activity', member })}
                              >
                                View Activity
                              </DropdownMenuItem>
                              {member.status === 'invited' && (
                                <DropdownMenuItem
                                  onClick={() => handleMemberAction({ type: 'resend_invitation', member })}
                                >
                                  Resend Invitation
                                </DropdownMenuItem>
                              )}
                              <DropdownMenuSeparator />
                              <DropdownMenuItem
                                onClick={() => handleMemberAction({ type: 'remove', member })}
            ...
            {filteredMembers.length === 0 && (
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-medium mb-2">
                  {filter === 'all' ? 'No family members yet' : `No ${filter} members`}
                </h3>
