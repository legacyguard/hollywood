...
export function BulkImportSummary({
  ...
  const categoryStats = categorizations.reduce((acc, cat) => {
    acc[cat.type] = (acc[cat.type] || 0) + 1;
    return acc;
  }, {} as Record<DocumentType, number>);

  // Calculate family relevance distribution
  const relevanceStats = categorizations.reduce((acc, cat) => {
  ...
  return (
    ...
    >
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-green-700">
                {documents.length} documents have been securely imported and organized for your family's protection.
              </p>
            </div>
          </div>
        </CardContent>
            ...
            {Object.entries(categoryStats).map(([type, count]) => (
              <div key={type} className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Badge className={cn('text-xs font-medium', getDocumentTypeColor(type as DocumentType))}>
          ...
          <div className="space-y-4">
            {relevanceStats.high > 0 && (
              <div className="flex items-start gap-3 p-3 bg-red-50 rounded-lg">
                ...
                <div className="flex-1">
                  <h4 className="font-medium text-red-900">Review High-Priority Documents</h4>
                  <p className="text-sm text-red-700 mt-1">
                    {relevanceStats.high} critical documents need your attention. Consider professional legal review for estate planning documents.
                  </p>
                </div>
              </div>
            )}
            ...
            {categoryStats.will || categoryStats.trust ? (
              <div className="flex items-start gap-3 p-3 bg-purple-50 rounded-lg">
                ...
                <div className="flex-1">
                  <h4 className="font-medium text-purple-900">Consider Professional Review</h4>
                  <p className="text-sm text-purple-700 mt-1">
                    Estate planning documents benefit from attorney review to ensure legal compliance.
                  </p>
                </div>
              </div>
            ) : null}
            ...
            <div className="flex items-start gap-3 p-3 bg-blue-50 rounded-lg">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-blue-700 mt-1">
                  Invite trusted family members to access relevant documents for better family protection.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3 p-3 bg-green-50 rounded-lg">
              ...
              <div className="flex-1">
                <h4 className="font-medium text-green-900">Set up Document Monitoring</h4>
                <p className="text-sm text-green-700 mt-1">
                  We'll help you track expiry dates and remind you when documents need updating.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-between">
        <div className="flex-1">
          <h3 className="font-medium mb-2">Your family's protection level just increased!</h3>
          <p className="text-sm text-muted-foreground">
            These {documents.length} documents are now securely organized and accessible to your designated family members when needed.
          </p>
        </div>
