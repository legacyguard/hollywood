...
interface TreeBranch {
  ...
  milestoneType?: 'foundation' | 'family' | 'protection' | 'legacy';
}
...
export const AdaptiveLegacyTree: React.FC<AdaptiveLegacyTreeProps> = ({
  ...
  const [treeHealth, setTreeHealth] = useState<'dormant' | 'growing' | 'thriving' | 'flourishing'>('dormant');
  ...
  useEffect(() => {
    const totalProgress = documentsCount + (guardiansCount * 2) + familyMembersCount +
                         Math.floor(willProgress / 20) + (timeCapsules * 1.5);
  ...
  }, [documentsCount, guardiansCount, familyMembersCount, willProgress, timeCapsules]);
  ...
  const generateTreeStructure = useCallback(() => {
    ...
    if (documentsCount > 0) {
      ...
      const foundationBranch: TreeBranch = {
        id: 'foundation',
        level: 1,
        angle: effectiveMode === 'empathetic' ? 45 : effectiveMode === 'pragmatic' ? 30 : 35,
    ...
    if (familyMembersCount > 0) {
      ...
      const familyBranch: TreeBranch = {
        id: 'family',
        level: 1,
        angle: effectiveMode === 'empathetic' ? -60 : effectiveMode === 'pragmatic' ? -40 : -50,
    ...
    if (guardiansCount > 0) {
      ...
      const protectionBranch: TreeBranch = {
        id: 'protection',
        level: 1,
        angle: effectiveMode === 'pragmatic' ? 15 : effectiveMode === 'empathetic' ? 25 : 20,
    ...
    if (timeCapsules > 0) {
      ...
      const legacyBranch: TreeBranch = {
        id: 'legacy',
        level: 1,
        angle: effectiveMode === 'empathetic' ? -30 : effectiveMode === 'pragmatic' ? -20 : -25,
    ...
    newBranches.forEach(branch => {
      if (branch.hasLeaves && branch.level === 1) {
        const leafCount = effectiveMode === 'empathetic' ?
        ...
        for (let i = 0; i < leafCount; i++) {
          const leafType = branch.hasFruit ? 'fruit' : branch.hasFlowers ? 'flower' : 'leaf';
          newLeaves.push({
            id: `leaf-${leafId++}`,
            branchId: branch.id,
  ...
  // Helper functions
  const getLeafColor = (type: TreeLeaf['type'], milestoneType?: TreeBranch['milestoneType'], mode: PersonalityMode = 'adaptive') => {
    if (type === 'flower') {
      switch (mode) {
        case 'empathetic': return 'text-pink-400';
        case 'pragmatic': return 'text-purple-500';
    ...
    switch (milestoneType) {
      case 'foundation': return mode === 'empathetic' ? 'text-green-400' : 'text-green-600';
      case 'family': return mode === 'empathetic' ? 'text-emerald-300' : 'text-emerald-500';
      case 'protection': return mode === 'pragmatic' ? 'text-blue-600' : 'text-blue-500';
      case 'legacy': return 'text-violet-500';
      default: return 'text-green-500';
    }
  };

  const getLeafMilestone = (milestoneType?: TreeBranch['milestoneType'], index: number = 0) => {
    switch (milestoneType) {
      case 'foundation': return `Document protection ${index + 1}`;
      case 'family': return `Family member ${index + 1} protected`;
      case 'protection': return `Guardian ${index + 1} assigned`;
      case 'legacy': return `Time capsule ${index + 1} created`;
  ...
  const renderBranch = (branch: TreeBranch) => {
    ...
    return (
      <motion.path
        ...
        strokeLinecap="round"
        fill="none"
        initial={!shouldReduceMotion ? { pathLength: 0, opacity: 0 } : undefined}
  ...
  const getBranchColor = (branch: TreeBranch) => {
    ...
    switch (branch.milestoneType) {
      case 'foundation': return effectiveMode === 'pragmatic' ? '#059669' : '#10b981';
      case 'family': return effectiveMode === 'empathetic' ? '#ec4899' : '#f59e0b';
      case 'protection': return effectiveMode === 'pragmatic' ? '#2563eb' : '#3b82f6';
      case 'legacy': return '#8b5cf6';
  ...
  const renderLeaf = (leaf: TreeLeaf) => {
    ...
    const LeafIcon = leaf.type === 'flower' ? Heart : leaf.type === 'fruit' ? Star : Leaf;
    ...
    return (
      <motion.g key={leaf.id}>
        <motion.foreignObject
          ...
          onMouseEnter={() => interactive && showTooltips && setHoveredElement(leaf.id)}
          onMouseLeave={() => setHoveredElement(null)}
        >
        ...
        {effectiveMode === 'empathetic' && !shouldReduceMotion && leaf.animate && (
          <motion.foreignObject
            x={leafX - 8}
            y={leafY - 8}
            width={16}
  ...
  return (
    <div className={`relative ${className}`}>
      <svg
        ...
        style={{ filter: effectiveMode === 'empathetic' ? 'drop-shadow(0 4px 6px rgba(0, 0, 0, 0.1))' : 'none' }}
      >
        {/* Tree branches */}
        ...
        {treeHealth === 'flourishing' && effectiveMode === 'empathetic' && !shouldReduceMotion && (
          <motion.circle
            cx={config.width / 2}
        ...
        >
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="font-medium text-gray-700">
              {treeHealth === 'flourishing' ? 'Flourishing' :
               treeHealth === 'thriving' ? 'Thriving' :
               treeHealth === 'growing' ? 'Growing' : 'Beginning'}
            </span>
            {effectiveMode === 'empathetic' && <Heart className="w-3 h-3 text-pink-400" />}
            {effectiveMode === 'pragmatic' && <Shield className="w-3 h-3 text-blue-500" />}
          </div>
        </motion.div>
      )}
