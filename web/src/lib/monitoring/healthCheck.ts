...
export class HealthCheckService {
  ...
  private async runSingleCheck(name: string, checkFn: HealthCheckFunction): Promise<HealthCheckResult> {
    ...
    throw lastError || new Error(`Health check ${name} failed after ${this.config.retries} attempts`);
  }
 ...
 */
healthCheckService.registerCheck('database', async (): Promise<HealthCheckResult> => {
  try {
 ...
 */
healthCheckService.registerCheck('localStorage', async (): Promise<HealthCheckResult> => {
  try {
 ...
 * Memory usage check
 */
healthCheckService.registerCheck('memory', async (): Promise<HealthCheckResult> => {
  try {
