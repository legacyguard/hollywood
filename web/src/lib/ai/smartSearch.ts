...
export interface SearchFilters {
  ...
  dateRange?: {
    ...
    field: 'created' | 'modified' | 'expiration' | 'effective';
  };
  tags?: string[];
...
export interface SearchHighlight {
  field: 'title' | 'content' | 'metadata' | 'tags';
  excerpt: string;
  matchedTerms: string[];
...
export interface DocumentRecommendation {
  type: 'similar' | 'related' | 'completion' | 'update' | 'action';
  title: string;
  description: string;
...
export class SmartSearchService {
  ...
  async updateIndex(
    documentId: string,
    updates: Partial<Omit<SearchIndex, 'documentId' | 'lastIndexed'>>
  ): Promise<void> {
  ...
  async findSimilarDocuments(
    ...
    return similarities
      ...
      .slice(0, limit)
      .map(({ doc, score }) => this.convertToSearchResult(doc, score, ['similarity']));
  }
  ...
  async getRecommendations(
    userId?: string,
    context?: 'recent' | 'important' | 'expiring' | 'related',
    limit = 10
    ...
    if (expiringDocs.length > 0) {
      recommendations.push({
        ...
        description: 'These documents require attention due to upcoming expiration dates',
  ...
  getIndexStatistics(): {
    ...
    return {
      ...
      categories,
      averageDocumentSize: docs.length > 0 ? totalContentLength / docs.length : 0,
      lastIndexed
    };
  }
  ...
  private generateFacets(results: SearchResult[]): SearchFacets {
    ...
    for (const result of results) {
      ...
      if (result.lastModified > monthsAgo) {
        ...
        facets.dateRanges['this-year'] = (facets.dateRanges['this-year'] || 0) + 1;
      } else {
  ...
  private passesFilters(indexEntry: SearchIndex, filters: SearchFilters): boolean {
    ...
    if (filters.dateRange) {
      ...
      if (targetDate && (targetDate < filters.dateRange.start || targetDate > filters.dateRange.end)) {
        return false;
      }
    }
  ...
  private convertToSearchResult(
    ...
    return {
      ...
      lastModified: new Date(indexEntry.metadata.lastModified || indexEntry.lastIndexed),
  ...
  private calculateDocumentSimilarity(doc1: SearchIndex, doc2: SearchIndex): number {
    // Category similarity
    let similarity = 0;
    if (doc1.analysis.category.primary === doc2.analysis.category.primary) {
      similarity += 0.3;
      if (doc1.analysis.category.secondary === doc2.analysis.category.secondary) {
        similarity += 0.2;
      }
    }
  ...
  private getCategorySuggestions(partialQuery: string): QuerySuggestion[] {
    const categories = ['legal', 'financial', 'medical', 'insurance', 'property'];
    return categories
  ...
  private async findIncompleteDocumentSets(): Promise<DocumentRecommendation[]> {
...
export const indexDocument = (
  ...
  return smartSearchService.indexDocument(documentId, title, content, analysis, metadata);
};
