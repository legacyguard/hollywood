...
describe('EncryptionService', () => {
  ...
  describe('Initialization', () => {
    ...
    test('should initialize with password and existing salt', async () => {
      const password = 'test-password-123';
      const { saltB64: firstSalt } = await service.initializeWithPassword(password);
      
      service.clearKey();
      const { saltB64: secondSalt } = await service.initializeWithPassword(password, firstSalt);
  ...
  describe('Encryption & Decryption', () => {
    ...
    test('should throw error when encrypting without key', async () => {
      ...
      await expect(service.encrypt('test')).rejects.toThrow('Encryption key not initialized');
    });
    ...
    test('should throw error when decrypting without key', async () => {
      ...
      await expect(service.decrypt(encrypted)).rejects.toThrow('Encryption key not initialized');
    });
  });
