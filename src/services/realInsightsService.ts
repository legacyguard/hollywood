...
class RealInsightsService {
  ...
  async analyzeDocument(documentId: string, userId: string): Promise<DocumentAnalysis> {
    try {
      ...
      const insights = await this.generateDocumentInsights(document, extractedValue);
      const recommendations = await this.generateRecommendations(document, extractedValue);
      ...
      return {
        ...
        extractedValue,
        insights: {
          immediate: storedInsights.filter(i => insights.immediate.some(orig => orig.title === i.title)),
          potential: storedInsights.filter(i => insights.potential.some(orig => orig.title === i.title))
        },
        recommendations
      };
    } catch (_error) {
  ...
  async generateFamilyImpactStatement(userId: string): Promise<FamilyImpactStatement> {
    try {
      ...
      const protectionGaps = await this.analyzeProtectionGaps(documents, familyMembers);
      const affectedMembers = await this.analyzeAffectedMembers(familyMembers, documents);
      ...
      const statement: FamilyImpactStatement = {
        ...
        impactDescription: this.generateImpactDescription(protectionLevel, familyMembers.length),
        affectedMembers,
        protectionGaps,
        overallProtectionLevel: protectionLevel,
        estimatedTimeSaved: timeSaved,
        emotionalBenefits: this.generateEmotionalBenefits(protectionLevel, familyMembers.length),
  ...
  // Database Operations
  async getQuickInsights(userId: string, filters?: InsightFilters): Promise<{ data: QuickInsight[]; total: number }> {
    try {
      let query = supabase
        .from('quick_insights')
        .select('*', { count: 'exact' })
  ...
  async storeInsight(insight: Omit<QuickInsight, 'id' | 'createdAt' | 'updatedAt'>): Promise<QuickInsight> {
    try {
  ...
  async storeMultipleInsights(insights: Omit<QuickInsight, 'id' | 'createdAt' | 'updatedAt'>[]): Promise<QuickInsight[]> {
    try {
  ...
  async updateInsight(insightId: string, updates: Partial<QuickInsight>): Promise<QuickInsight> {
    try {
      ...
      if (updates.actionable !== undefined) updateData.actionable = updates.actionable;
  ...
  async getInsightAnalytics(userId: string, timeframe: { start: string; end: string }): Promise<InsightAnalytics> {
    try {
      ...
      const topCategories = Array.from(categoryMap.entries())
        ...
        .map(([category, count]) => ({
          category,
          count,
          averageImpact: this.calculateAverageImpact(allInsights.filter(i => i.metadata?.category === category))
        }));
  ...
  private async generateDocumentInsights(document: any, extractedValue: any) {
    const immediate: Omit<QuickInsight, 'id' | 'createdAt' | 'updatedAt'>[] = [];
    const potential: Omit<QuickInsight, 'id' | 'createdAt' | 'updatedAt'>[] = [];
  ...
  private calculateProtectionLevel(documents: any[], familyMembers: any[]): number {
  ...
  private generateTrendData(insights: any[], timeframe: { start: string; end: string }) {
  ...
  private async storeFamilyImpactStatement(statement: FamilyImpactStatement): Promise<void> {
    try {
      ...
      await this.storeInsight({
        ...
        impact: statement.overallProtectionLevel > 75 ? 'high' : statement.overallProtectionLevel > 50 ? 'medium' : 'low',
        priority: 'important',
        actionable: statement.protectionGaps.length > 0,
        actionText: statement.protectionGaps.length > 0 ? 'Address protection gaps' : undefined,
        metadata: {
  ...
  private generateScenario(documentCount: number, familyMemberCount: number): string {
  ...
  private analyzeAffectedMembers(familyMembers: any[], documents: any[]) {
    return familyMembers.map(member => ({
      memberId: member.id,
      name: member.name,
      impact: documents.length > 3 ? 'High security and clear guidance' : 'Basic protection established',
      riskLevel: documents.length > 3 ? 'low' as const : documents.length > 1 ? 'medium' as const : 'high' as const
    }));
  }
