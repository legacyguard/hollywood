...
export const MyFamilyPage: React.FC<MyFamilyPageProps> = ({
  ...
  const getFamilyInsights = useCallback(() => {
    const insights = {
      ...
      guardiansAssigned: mockFamilyMembers.filter(m => m.roles.isGuardian).length,
      executorAssigned: mockFamilyMembers.some(m => m.roles.isExecutor),
      legacyMessages: mockFamilyMembers.filter(m => m.roles.hasLegacyMessages).length,
  ...
  const renderFamilyMemberCard = (member: FamilyMember) => (
    <Card 
      ...
      className={`cursor-pointer transition-all hover:shadow-lg ${
        ...
        member.status === 'partial' ? 'border-yellow-200 bg-yellow-50' :
      ...
      <CardContent className="p-4">
        <div className="flex items-start justify-between mb-3">
          <div>
            ...
            {member.dateOfBirth && (
              <p className="text-xs text-gray-500">Born: {member.dateOfBirth}</p>
            )}
          </div>
          <div className="flex flex-col items-end space-y-1">
            {member.status === 'complete' && <CheckCircle className="h-5 w-5 text-green-600" />}
            {member.status === 'partial' && <AlertCircle className="h-5 w-5 text-yellow-600" />}
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">
              Manage your family members and assign their roles in your estate plan
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        <Card>
          <CardContent className="p-4 text-center">
            ...
            <div className="text-2xl font-bold">{insights.guardiansAssigned}</div>
