    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Badge className={`gap-2 ${currentPlan === 'premium' ? 'bg-purple-100 text-purple-800' : currentPlan === 'family' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}`}>
      ...
      {(isNearMemberLimit() || needsPlanUpgrade()) && (
        <Alert className={needsPlanUpgrade() ? "border-red-200 bg-red-50" : "border-yellow-200 bg-yellow-50"}>
          <AlertTriangle className={`h-4 w-4 ${needsPlanUpgrade() ? 'text-red-600' : 'text-yellow-600'}`} />
          ...
          <Card className="max-w-2xl mx-auto">
            <CardContent className="p-12">
              ...
              <p className="text-muted-foreground mb-6 max-w-md mx-auto">
                Start building your family's protection network by inviting your loved ones.
                Together, you can share important documents, coordinate schedules, and ensure everyone is prepared for life's moments.
              </p>
              ...
              <Button size="lg" className="gap-2" onClick={() => setActiveTab('members')}>
      ...
      {familyMembers.length > 0 && (
        <Tabs value={activeTab} onValueChange={setActiveTab}>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <span className="text-3xl font-bold">{protectionStatus.protectionLevel}%</span>
                        <Badge className={protectionStatus.protectionLevel >= 80 ? 'bg-green-100 text-green-800' : protectionStatus.protectionLevel >= 60 ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}>
                          {protectionStatus.protectionLevel >= 80 ? 'Excellent' : protectionStatus.protectionLevel >= 60 ? 'Good' : 'Needs Work'}
                        </Badge>
                      </div>
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm">
                          ...
                          <span>{protectionStatus.activeMembers}/{protectionStatus.totalMembers}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                        ...
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="flex items-center gap-1">
                            ...
                            {protectionStatus.emergencyContactsSet ? 'Ready' : 'Setup needed'}
                          </span>
                        </div>
                      </div>
                    </div>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {protectionStatus.recommendations.map((recommendation, index) => (
          ...
          <TabsContent value="calendar">
            {user && <SharedFamilyCalendar userId={user.id} familyMembers={familyMembers} />}
          </TabsContent>

          <TabsContent value="sharing">
            {user && <FamilyDocumentSharing userId={user.id} familyMembers={familyMembers} />}
          </TabsContent>
        </Tabs>
      )}
    </div>
