...
export const SealOfTrust: React.FC<SealOfTrustProps> = ({
    ...
    return (
      <div className={`flex items-center gap-2 text-xs text-muted-foreground ${className}`}>
        <div className="w-4 h-4 bg-primary/10 rounded-full flex items-center justify-center">
          <Icon name="shield-check" className="w-2.5 h-2.5 text-primary" />
        </div>
  ...
  if (variant === 'pdf') {
    return (
      <div className={`border-t border-gray-300 pt-4 mt-8 text-xs text-gray-600 ${className}`}>
        <div className="flex items-start gap-3">
          <div className="w-8 h-8 border-2 border-primary rounded-full flex items-center justify-center flex-shrink-0">
            <Icon name="shield-check" className="w-4 h-4 text-primary" />
          </div>
          <div className="flex-1">
            <div className="font-semibold mb-1">LegacyGuard Legal Certification</div>
            <p className="mb-2">
              {getComplianceText()} {finalCreatedAt}.
            </p>
            <div className="flex items-center gap-4 text-xs">
              <div>
                <span className="font-medium">Document ID:</span> {finalDocumentId}
              </div>
              <div>
  ...
  return (
    <div className={`bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/10 dark:to-indigo-900/10 border border-blue-200 dark:border-blue-800 rounded-lg p-4 ${className}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-blue-700 dark:text-blue-400">
            <div className="flex items-center gap-2">
              ...
              <span><strong>Generated:</strong> {finalCreatedAt}</span>
            </div>
            <div className="flex items-center gap-2">
              <Icon name="hash" className="w-3 h-3" />
              <span><strong>Doc ID:</strong> {finalDocumentId.slice(0, 12)}...</span>
            </div>
            <div className="flex items-center gap-2">
