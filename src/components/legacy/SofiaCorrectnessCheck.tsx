...
export const SofiaCorrectnessCheck: React.FC<SofiaCorrectnessCheckProps> = ({
  ...
  const performChecks = (): CheckItem[] => {
    ...
    if (totalPercentage === 100) {
      checks.push({
        ...
        description: 'Perfect! Your inheritance distribution is complete and balanced.'
      });
      ...
      checks.push({
        ...
        description: 'Please adjust the percentages so they total exactly 100%.',
        actionText: 'Fix Now',
      ...
      checks.push({
        ...
        description: 'Consider what happens to the remaining percentage of your estate.',
        actionText: 'Review',
        onAction: () => onGoToStep('beneficiaries')
      });
    } else {
    ...
    if (willData.executor_data.primaryExecutor?.name) {
      ...
      checks.push({
        ...
        description: 'Consider appointing someone to manage your estate administration.',
    ...
    const beneficiariesMissingInfo = willData.beneficiaries.filter(b => !b.name || b.name.trim() === '');
    ...
    if (fullName && dateOfBirth && address) {
      ...
      checks.push({
        ...
        description: 'Complete personal details are required for legal validity.',
        actionText: 'Complete Now',
    ...
    if (willType === 'holographic') {
      checks.push({
        ...
        description: 'Remember: You must copy this entire document by hand and sign it yourself. No witnesses needed.',
      });
    } else {
      checks.push({
        id: 'will_type_requirements',
        ...
        description: 'Remember: You must sign this in the presence of two qualified witnesses who are not beneficiaries.',
      });
    }
    ...
    if (hasAssets) {
      checks.push({
        ...
        description: 'Your asset inventory will help with estate administration.'
      });
    } else {
      checks.push({
        id: 'assets',
        ...
        description: 'Consider adding details about your property, accounts, and belongings.',
  ...
  const getStatusIcon = (status: CheckItem['status']) => {
    switch (status) {
      case 'success':
        return <Icon name="check-circle" className="w-5 h-5 text-green-600" />;
      case 'warning':
        return <Icon name="alert-circle" className="w-5 h-5 text-yellow-600" />;
      case 'error':
        return <Icon name="x-circle" className="w-5 h-5 text-red-600" />;
    }
  };
      ...
      <div className="space-y-4">
        {checks.map((check) => (
          <Card key={check.id} className={`p-4 ${
            check.status === 'error' ? 'border-red-200 bg-red-50 dark:bg-red-900/20' :
