...
import { Scale, Shield, CheckCircle, Clock, Star, ArrowRight, Sparkles } from 'lucide-react';
...
interface ProfessionalReviewButtonProps {
  ...
  onReviewRequested?: (request: Omit<ReviewRequest, 'id' | 'user_id' | 'created_at' | 'updated_at'>) => void;
  className?: string;
}
...
export function ProfessionalReviewButton({
  ...
  const getRecommendedReviewType = () => {
    if (familyContext.complex_assets || familyContext.business_interests) {
      return 'certified';
    } else if (familyContext.family_members_count > 2 || familyContext.minor_children) {
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-blue-100 text-sm">
                    Get your {documentType.toLowerCase()} reviewed by licensed attorneys
                  </p>
                </div>
              </div>

              <div className="text-right">
                <div className="text-2xl font-bold mb-1">+{getTrustScoreBoost()}</div>
              ...
              <div className="flex-1 space-y-3">
                <div>
                  ...
                  <p className="text-gray-600 text-sm leading-relaxed">
                    Have your {documentType.toLowerCase()} reviewed by licensed estate planning attorneys.
                    Boost your trust score by +{getTrustScoreBoost()} points and ensure legal compliance.
                  </p>
                </div>
...
export function WillWizardProfessionalReview({
  ...
  const familyContext = {
    ...
    minor_children: willData?.beneficiaries?.some((b: any) => b.isMinor) || false,
    complex_assets: willData?.assets?.some((a: any) => a.type === 'business' || a.type === 'investment') || false,
    business_interests: willData?.assets?.some((a: any) => a.type === 'business') || false
  };
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-amber-700 text-sm leading-relaxed mb-3">
              Your will contains important provisions for your family's future. A professional
              review by a licensed attorney can ensure everything is legally sound and optimized
              for your family's protection.
            </p>

            {(familyContext.complex_assets || familyContext.business_interests || familyContext.minor_children) && (
              <div className="mb-3">
                <p className="text-amber-700 text-sm font-medium mb-2">Recommended because you have:</p>
                <ul className="text-xs text-amber-600 space-y-1">
                  {familyContext.minor_children && (
                    <li>• Minor children requiring special guardianship provisions</li>
                  )}
