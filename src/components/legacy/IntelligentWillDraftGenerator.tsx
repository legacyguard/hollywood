...
export const IntelligentWillDraftGenerator: React.FC<IntelligentWillDraftGeneratorProps> = ({
  onDraftAccepted,
  onStartFromScratch,
  willType
}) => {
  ...
  const analyzeUserDataForWill = (
    ...
    if (children.length > 0) {
      ...
      children.forEach(child => {
        ...
        totalPercentage += percentagePerChild;
      });
    }
    ...
    if (parents.length > 0 && spouses.length === 0 && children.length === 0) {
      ...
      parents.forEach(parent => {
        ...
        totalPercentage += percentagePerParent;
      });
    }
    ...
    analysis.suggestedAssetDistribution = bundles.map(bundle => {
      ...
      let reasoning = 'This asset will be distributed according to percentage shares.';
      ...
      if (bundleName.includes('home') || bundleName.includes('house') || bundleName.includes('byt') || bundleCategory.includes('real estate')) {
        if (spouses.length > 0) {
          suggestedRecipient = spouses[0].name;
          reasoning = 'Family home typically goes to surviving spouse for continued residence.';
        }
      } else if (bundleName.includes('car') || bundleName.includes('vehicle') || bundleName.includes('auto') || bundleCategory.includes('vehicle')) {
        if (children.length > 0) {
          suggestedRecipient = children[0].name;
          reasoning = 'Vehicle often goes to adult children who can use it immediately.';
        } else if (spouses.length > 0) {
          suggestedRecipient = spouses[0].name;
          reasoning = 'Vehicle will be useful for your spouse\'s daily needs.';
        }
      } else if (bundleCategory.includes('financial') || bundleCategory.includes('investment')) {
        if (spouses.length > 0) {
          suggestedRecipient = spouses[0].name;
          reasoning = 'Financial assets typically go to spouse for financial security.';
        }
      }
    ...
    if (nonBeneficiaryGuardians.length > 0) {
      ...
      analysis.suggestedExecutor = {
        ...
        reasoning: 'Someone outside direct beneficiaries can handle estate administration objectively.'
      };
    } else if (children.length > 0) {
      analysis.suggestedExecutor = {
        name: children[0].name,
        relationship: children[0].relationship,
        reasoning: 'Your eldest child can serve as executor if they are responsible adults.'
      };
    }
    ...
    if (guardians.length === 0) {
      analysis.missingInfo.push('No guardians/family members found in your account');
    }
    if (bundles.length === 0) {
      analysis.missingInfo.push('No document bundles found - consider organizing your assets');
    }
    if (analysis.suggestedBeneficiaries.length === 0) {
      analysis.missingInfo.push('Unable to identify potential beneficiaries from existing data');
    }

    // 6. Calculate Confidence
    if (guardians.length >= 3 && bundles.length >= 2 && analysis.suggestedBeneficiaries.length >= 2) {
      analysis.confidence = 'high';
    } else if (guardians.length >= 1 && (bundles.length >= 1 || analysis.suggestedBeneficiaries.length >= 1)) {
        ...
        >
          ...
          <Card className="p-6 bg-gradient-to-r from-primary/5 to-blue/5 border-primary/20">
            <div className="flex items-start gap-4">
              <Icon name="magic-wand" className="w-6 h-6 text-primary mt-1" />
                  ...
                  {analysis.suggestedBeneficiaries.map((beneficiary, index) => (
                    <div key={index} className="flex items-start gap-3 p-3 bg-muted/50 rounded">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground">{beneficiary.reasoning}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-3">
                  {analysis.suggestedAssetDistribution.map((asset, index) => (
                    <div key={index} className="flex items-start gap-3 p-3 bg-muted/50 rounded">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="font-medium">{asset.bundleName}</span>
                          <Icon name="arrow-right" className="w-3 h-3 text-muted-foreground" />
                          <span className="text-primary">{asset.suggestedRecipient}</span>
                        </div>
                        <p className="text-sm text-muted-foreground">{asset.reasoning}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <Card className="p-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <span className="font-medium">{analysis.suggestedExecutor.name}</span>
                      <Badge variant="secondary">{analysis.suggestedExecutor.relationship}</Badge>
                    </div>
                    <p className="text-sm text-muted-foreground">{analysis.suggestedExecutor.reasoning}</p>
                  </div>
                </Card>
              )}

              {analysis.suggestedGuardians && analysis.suggestedGuardians.length > 0 && (
                <Card className="p-4">
                  <h4 className="font-semibold mb-2 flex items-center gap-2">
                    ...
                    {analysis.suggestedGuardians.map((guardian, index) => (
                      <div key={index}>
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant="outline">{index === 0 ? 'Primary' : 'Backup'}</Badge>
                        </div>
                        <p className="text-xs text-muted-foreground mt-1">{guardian.reasoning}</p>
                      </div>
                    ))}
                  </div>
                </Card>
