...
export const CountrySelector: React.FC<CountrySelectorProps> = ({ onCountryConfirmed }) => {
  const { countryCode, jurisdiction, setCountryCode, isLoading } = useLocalization();
        ...
        <div className="max-w-4xl mx-auto px-6 py-16">
          <FadeIn duration={0.6}>
            <div className="text-center mb-12">
              <Icon name="globe" className="w-12 h-12 text-primary mx-auto mb-6" />
                ...
                <FadeIn key={country.code} duration={0.4} delay={0.1 * index}>
                  ...
                  >
                    <div className="text-center">
                      ...
                      <p className="text-sm text-muted-foreground mb-4">{country.description}</p>
                      <div className="text-xs text-primary font-medium">{country.domain}</div>
                      {country.code === countryCode && (
                        <div className="mt-4">
                          <Icon name="check-circle" className="w-5 h-5 text-green-600 mx-auto" />
                        </div>
                      )}
                    </div>
                  </Card>
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="max-w-4xl mx-auto px-6 py-16">
        <FadeIn duration={0.8}>
          <div className="text-center mb-12">
            <Icon name="shield-check" className="w-16 h-16 text-primary mx-auto mb-6" />
