...
import { willTemplateLibrary, type WillTemplate, type UserProfile, type TemplateCategory, type ComparisonReport } from '@/lib/will-template-library';
...
export const WillTemplateLibrary: React.FC<WillTemplateLibraryProps> = ({
  ...
  const getCategoryIcon = (category: TemplateCategory): string => {
    switch (category) {
      case 'young_professional': return 'briefcase';
      case 'new_parent': return 'baby';
      case 'established_family': return 'home';
      case 'blended_family': return 'users';
      case 'business_owner': return 'building-office';
      case 'retiree': return 'user-check';
      case 'single_parent': return 'user';
      case 'childless_couple': return 'heart';
      case 'charitable_focused': return 'gift';
      case 'international': return 'globe';
      case 'special_needs': return 'heart-handshake';
      default: return 'document';
    }
  };

  const getComplexityColor = (complexity: WillTemplate['complexity']): string => {
    switch (complexity) {
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Choose from professionally crafted templates designed for different life situations
        </p>
      </div>
          ...
          {recommendedTemplates.length > 0 ? (
            ...
            <Card className="p-8 text-center">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                We need more information about your situation to provide personalized recommendations.
              </p>
          ...
          {categories.map(({ category, count, description }) => (
            <div key={category} className="space-y-3">
              <div className="flex items-center gap-3">
                <Icon name={getCategoryIcon(category)} className="w-5 h-5 text-primary" />
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 ml-8">
                {willTemplateLibrary.getTemplatesByCategory(category).map((template) => (
                  <TemplateCard
                    key={template.id}
                    template={template}
                    compact={true}
            ...
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-3">
                  <Icon name={getCategoryIcon(selectedTemplate.category)} className="w-6 h-6 text-primary" />
          ...
          <div className="space-y-4">
            <div className="p-4 border rounded-lg bg-yellow-50 border-yellow-200">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="font-medium text-yellow-900">Existing Will Data Found</h4>
                  <p className="text-sm text-yellow-800 mt-1">
                    You have existing will information. Applying this template will modify your current data.
                    We'll show you exactly what changes before finalizing.
                  </p>
                </div>
              </div>
            </div>
    ...
    <Card className={`p-4 hover:shadow-md transition-shadow ${isRecommended ? 'border-primary/50 bg-primary/5' : ''}`}>
      ...
      {!compact && (
        <div className="space-y-3 mb-4">
          <div>
            <h5 className="text-xs font-medium text-muted-foreground mb-1">Key Features</h5>
            <div className="flex flex-wrap gap-1">
              {template.preview.keyFeatures.slice(0, 3).map((feature, index) => (
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          <Badge variant="outline">{template.estimatedCompletionTime} minutes</Badge>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <span className="ml-2 capitalize">{template.targetProfile.maritalStatus}</span>
              </div>
              <div>
                <span className="font-medium">Has Children:</span>
                <span className="ml-2">{template.targetProfile.hasChildren ? 'Yes' : 'No'}</span>
              </div>
              <div>
              ...
              <div>
                ...
                <span className="ml-2 capitalize">{template.targetProfile.estimatedNetWorth}</span>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-3 text-sm">
              <div className="flex items-center justify-between p-2 bg-muted/50 rounded">
                ...
                <Badge variant="outline">{template.preview.beneficiaryCount}</Badge>
              </div>
              <div className="flex items-center justify-between p-2 bg-muted/50 rounded">
              ...
              <div className="space-y-2">
                {template.legalNotices.map((notice, index) => (
                  <div key={index} className="flex items-start gap-2 text-sm">
                    <Icon name="info" className="w-4 h-4 text-blue-600 flex-shrink-0 mt-0.5" />
