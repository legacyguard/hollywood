      ...
      <div className="font-serif text-sm leading-relaxed text-gray-900 dark:text-gray-100">
        ...
        <div className="mb-6">
          ...
          <div className={`${currentStep === 'beneficiaries' ? 'bg-yellow-50 dark:bg-yellow-900/20 p-3 rounded-lg border-l-4 border-yellow-400' : ''}`}>
            {willData.beneficiaries.filter(b => b.relationship === 'child').length > 0 ? (
              ...
              <p>I have {currentStep === 'beneficiaries' ? '[children to be specified]' : 'no children'}.</p>
            )}
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className={`${currentStep === 'assets' ? 'bg-yellow-50 dark:bg-yellow-900/20 p-3 rounded-lg border-l-4 border-yellow-400' : ''}`}>
                ...
                <ul className="list-disc list-inside ml-4">
                  {willData.assets.realEstate.map((property, idx) => (
                    <li key={idx}>
                      ...
                      {property.value && `, estimated value: ${formatCurrency(property.value)}`}
                    </li>
                  ))}
                </ul>
              </div>
            ...
            {/* Bank Accounts */}
            {willData.assets.bankAccounts && willData.assets.bankAccounts.length > 0 ? (
              <div className="mb-4">
            ...
            {/* Personal Property */}
            {willData.assets.personalProperty && willData.assets.personalProperty.length > 0 ? (
              <div className="mb-4">
                ...
                <ul className="list-disc list-inside ml-4">
                  {willData.assets.personalProperty.map((item, idx) => (
                    <li key={idx}>
                      {item.description}
                      {item.value && `, estimated value: ${formatCurrency(item.value)}`}
              ...
              willData.beneficiaries.map((beneficiary, idx) => (
                <div key={beneficiary.id} className="mb-4 p-3 border border-gray-200 rounded">
                  ...
                  <ul className="list-disc list-inside ml-4 mt-2">
                    ...
                    {beneficiary.specificGifts && beneficiary.specificGifts.length > 0 && (
                      <li>The following specific assets: {beneficiary.specificGifts.join(', ')}</li>
                    )}
                    {beneficiary.conditions && (
                      <li>Subject to the following conditions: {beneficiary.conditions}</li>
                    )}
                  </ul>
                </div>
              ))
          ...
          <div className={`${currentStep === 'wishes' ? 'bg-yellow-50 dark:bg-yellow-900/20 p-3 rounded-lg border-l-4 border-yellow-400' : ''}`}>
            {willData.special_instructions.funeralWishes && (
              <div className="mb-3">
                ...
                <p className="text-sm">{willData.special_instructions.funeralWishes}</p>
              </div>
            )}
            ...
            {willData.special_instructions.digitalAssets && (
              <div className="mb-3">
                ...
                <p className="text-sm">{willData.special_instructions.digitalAssets}</p>
              </div>
            )}
            ...
            {willData.special_instructions.personalMessages && willData.special_instructions.personalMessages.length > 0 && (
              <div className="mb-3">
                ...
                {willData.special_instructions.personalMessages.map((message, idx) => (
            ...
            {currentStep === 'wishes' && 
             ...
             (!willData.special_instructions.personalMessages || willData.special_instructions.personalMessages.length === 0) && (
              <p className="text-gray-600 dark:text-gray-400 italic">
                [Your special wishes and instructions will appear here as you add them]
              </p>
            )}
          </div>
        </div>
          ...
          {willType === 'holographic' ? (
            <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
              ...
              <p className="text-sm mb-4">
                This entire Will has been written in my own handwriting and is signed by me on this day.
              </p>
              <div className="border-t border-gray-300 pt-4">
                <p className="text-sm mb-2">Signature: ________________________</p>
                <p className="text-sm mb-2">{willData.testator_data.fullName || '[Your Name]'}</p>
            ...
            <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg">
              ...
              <div className="border-t border-gray-300 pt-4 space-y-4">
                <div>
                  <p className="text-sm">Testator's Signature: ________________________    Date: ____________________</p>
                  <p className="text-sm">{willData.testator_data.fullName || '[Your Name]'}</p>
                </div>
                <div>
                  <p className="font-medium text-sm mb-2">WITNESSES:</p>
                  <div className="space-y-3">
                    <div>
                      <p className="text-sm">Witness 1 Signature: ________________________    Date: ____________________</p>
                      <p className="text-sm">Print Name: ________________________</p>
                      <p className="text-sm">Address: ________________________</p>
                    </div>
                    <div>
                      <p className="text-sm">Witness 2 Signature: ________________________    Date: ____________________</p>
                      <p className="text-sm">Print Name: ________________________</p>
                      <p className="text-sm">Address: ________________________</p>
                    </div>
                  </div>
