...
export interface GenerationalDocument {
  id: string;
  name: string;
  type: 'will' | 'photo' | 'letter' | 'certificate' | 'medical' | 'financial' | 'property' | 'story' | 'video' | 'audio';
  ...
  metadata: {
    ...
    expiryDate?: string;
    legalStatus?: 'draft' | 'active' | 'superseded' | 'archived';
  };
  versions: Array<{
    id: string;
    version: number;
  ...
  sharingHistory: Array<{
    id: string;
    action: 'shared' | 'viewed' | 'downloaded' | 'edited';
    userId: string;
    timestamp: string;
    details?: string;
  }>;
...
interface MultiGenerationalDocumentSharingProps {
  ...
  onShareDocument?: (docId: string, memberIds: string[], permissions: string[]) => void;
  onUpdatePermissions?: (docId: string, permissions: GenerationalDocument['permissions']) => void;
  onDownloadDocument?: (docId: string) => void;
  onViewDocument?: (docId: string) => void;
  onUploadDocument?: (file: File, metadata: Partial<GenerationalDocument['metadata']>) => void;
}

export const MultiGenerationalDocumentSharing: React.FC<MultiGenerationalDocumentSharingProps> = ({
  documents,
  familyMembers,
  currentUserId,
  onShareDocument,
  ...
  const canPerformAction = (doc: GenerationalDocument, action: 'view' | 'edit' | 'share' | 'download') => {
    switch (action) {
      case 'view':
        return doc.permissions.canView.includes(currentUserId) || doc.originalUploader === currentUserId;
      case 'edit':
        return doc.permissions.canEdit.includes(currentUserId) || doc.originalUploader === currentUserId;
      case 'share':
        return doc.permissions.canShare.includes(currentUserId) || doc.originalUploader === currentUserId;
      case 'download':
        return doc.permissions.canDownload.includes(currentUserId) || doc.originalUploader === currentUserId;
      default:
        return false;
    }
  };
  ...
  const getGenerationLabel = (generation: number) => {
    ...
    if (generation > 0) return `${generation} Generation${generation > 1 ? 's' : ''} Down`;
  ...
  const handleShareDocument = () => {
    if (selectedDoc && selectedMembers.length > 0 && selectedPermissions.length > 0) {
                  ...
                  <Card key={doc.id} className="p-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2">
                          {React.createElement(getDocumentIcon(doc.type), { className: "h-5 w-5 text-gray-600" })}
                          <div>
                            <h4 className="font-medium">{doc.name}</h4>
                            <p className="text-sm text-gray-600">
                              {getGenerationLabel(doc.generation)} • {new Date(doc.uploadDate).toLocaleDateString()}
                            </p>
                          </div>
                        </div>
