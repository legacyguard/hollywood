...
interface FamilyMember {
  ...
  email: string;
  avatar?: string;
  role: 'head' | 'parent' | 'child' | 'guardian';
  isOnline: boolean;
  lastSeen?: Date;
  timezone: string;
}
...
interface Channel {
  ...
  name: string;
  description: string;
  type: 'general' | 'announcements' | 'emergency' | 'planning' | 'private';
  participants: string[];
  createdBy: string;
  createdAt: Date;
  lastActivity: Date;
...
interface Announcement {
  ...
  category: 'general' | 'health' | 'financial' | 'legal' | 'family-event';
  readBy: string[];
  isPinned: boolean;
  expiresAt?: Date;
}
...
export const FamilyCommunicationCenter: React.FC<FamilyCommunicationCenterProps> = ({
  familyMembers = [],
  onMessageSent,
  existingChannels = [],
  ...
  const [announcements, setAnnouncements] = useState<Announcement[]>(existingAnnouncements);
  ...
  const sendMessage = (content: string, type: Message['type'] = 'text') => {
    ...
    setChannels(channels.map(c => c.id === activeChannel.id ? updatedChannel : c));
  ...
  const createChannel = (name: string, description: string, type: Channel['type']) => {
    const newChannel: Channel = {
      id: `channel-${Date.now()}`,
      name,
      description,
      type,
      participants: type === 'private' ? [currentUserId] : allMembers.map(m => m.id),
      createdBy: currentUserId,
      createdAt: new Date(),
      lastActivity: new Date(),
      isPinned: false,
  ...
  const createAnnouncement = (title: string, content: string, priority: Announcement['priority']) => {
  ...
  const getChannelTypeIcon = (type: Channel['type']) => {
    switch (type) {
      ...
      case 'announcements': return <Bell className="h-4 w-4" />;
      case 'emergency': return <Bell className="h-4 w-4 text-red-500" />;
      ...
      <div className="w-80 border-r bg-gray-50 flex flex-col">
        ...
        <div className="p-4 border-b">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-bold text-gray-900">Family Communication</h2>
        ...
        {activeChannel ? (
                  ...
                  {filteredMessages.map((message, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className={`flex-1 max-w-lg ${message.senderId === currentUserId ? 'flex justify-end' : ''}`}>
                                ...
                                return (
                                  <button
                                    ...
                                    className={`text-xs px-2 py-1 rounded-full border ${
                                      userReacted ? 'bg-blue-100 border-blue-300' : 'bg-white border-gray-200'
                                    }`}
                                  >
                                    {emoji} {count}
                                  </button>
                ...
                <div className="mb-3 p-2 bg-gray-50 rounded text-sm">
                  <div className="flex items-center justify-between">
                    <span className="text-gray-600">
                      Replying to {filteredMessages.find(m => m.id === replyingTo)?.senderName}
                    </span>
          ...
          <div className="flex-1 flex items-center justify-center text-gray-500">
            <div className="text-center">
              ...
              <p className="text-gray-600">Choose a channel from the sidebar to start communicating</p>
            </div>
          </div>
        )}
      </div>
