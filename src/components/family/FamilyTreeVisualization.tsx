...
export interface FamilyMember {
  ...
  roles: Array<'heir' | 'guardian' | 'executor' | 'trustee' | 'power_of_attorney'>;
}
...
interface FamilyTreeVisualizationProps {
  ...
  onExportTree?: (format: 'png' | 'pdf' | 'json') => void;
  onShareTree?: () => void;
...
export const FamilyTreeVisualization: React.FC<FamilyTreeVisualizationProps> = ({
  ...
  const currentUser = useMemo(() => {
    return members.find(member => member.id === currentUserId || member.relationship === 'self');
  ...
  const getMemberIcon = (member: FamilyMember) => {
    ...
    if (member.relationship === 'child' || member.relationship === 'grandchild') return Baby;
  ...
  const getMemberColor = (member: FamilyMember) => {
    if (member.relationship === 'self') return 'bg-primary text-primary-foreground';
    if (member.isGuardian) return 'bg-blue-100 text-blue-900 border-blue-300';
    if (member.roles.includes('heir')) return 'bg-green-100 text-green-900 border-green-300';
    if (member.status === 'invited') return 'bg-yellow-100 text-yellow-900 border-yellow-300';
    if (member.status === 'declined') return 'bg-red-100 text-red-900 border-red-300';
  ...
  const MemberCard = ({ member }: { member: FamilyMember }) => {
    ...
    return (
      ...
      >
        <Card className={`w-32 ${getMemberColor(member)} shadow-lg hover:shadow-xl transition-shadow`}>
  ...
  return (
    <div className="w-full h-full bg-gray-50">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setNewMember(prev => ({ ...prev, relationship: value as FamilyMember['relationship'] }))}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
      ...
      {/* Tree Visualization */}
      <div className={`relative overflow-auto ${isZoomed ? 'h-screen' : 'h-96'} bg-gradient-to-br from-blue-50 to-green-50`}>
        {viewMode === 'tree' && (
          <>
            <TreeConnections />
      ...
      <div className="bg-white border-t p-4">
        <div className="flex items-center justify-between text-sm text-gray-600">
          <div className="flex gap-6">
            ...
            <span>Heirs: {members.filter(m => m.roles.includes('heir')).length}</span>
          </div>
