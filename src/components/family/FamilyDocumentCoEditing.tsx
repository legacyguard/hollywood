...
interface SharedDocument {
  ...
  title: string;
  content: string;
  type: 'will' | 'letter' | 'instructions' | 'story' | 'other';
  ownerId: string;
...
export const FamilyDocumentCoEditing: React.FC<FamilyDocumentCoEditingProps> = ({
  familyMembers = [],
  onDocumentCreated,
  existingDocuments = []
}) => {
  ...
  const addCollaborator = (memberId: string) => {
    ...
    if (!member || activeDocument.collaborators.find(c => c.id === memberId)) return;
  ...
  const updateCollaboratorRole = (memberId: string, newRole: FamilyMember['role']) => {
    if (!activeDocument) return;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Family Document Co-Editing</h2>
          <p className="text-gray-600">Collaborate with family members on important documents</p>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <Button
                    size="sm"
                    variant={activeDocument.isLocked ? "destructive" : "outline"}
                    onClick={lockDocument}
                    className="gap-2"
                  >
                    {activeDocument.isLocked ? <Lock className="h-3 w-3" /> : <Unlock className="h-3 w-3" />}
              ...
              <div className={`${showComments ? 'col-span-8' : 'col-span-12'}`}>
                <div className="relative">
                  ...
                  <div className="absolute top-2 right-2 flex -space-x-2">
                    {activeDocument.collaborators.slice(0, 4).map((collaborator) => (
                      <div key={collaborator.id} className="relative">
                        <Avatar className={`w-8 h-8 border-2 ${collaborator.isOnline ? 'border-green-400' : 'border-gray-300'}`}>
                ...
                <div className="flex items-center justify-between mt-4 p-2 bg-gray-50 rounded-lg">
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                    ...
                    {editHistory.length} edits • Last saved {activeDocument.lastModified.toLocaleTimeString()}
                  </div>
                  <div className="flex gap-2">
                    <Dialog>
                      ...
                      <DialogContent>
                        ...
                        <div className="space-y-4">
                          <Select onValueChange={(value) => addCollaborator(value)}>
                            ...
                            <SelectContent>
                              {collaborators
                                .filter(m => !activeDocument.collaborators.find(c => c.id === m.id))
                                .map((member) => (
              ...
              <div className="mt-4 pt-4 border-t">
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {activeDocument.versions.map((version) => (
                    <div key={version.id} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg">
                      <div className="flex items-center gap-3">
                        <Avatar className="w-6 h-6">
                          <AvatarFallback className="text-xs">
                        ...
                        <div>
                          <div className="text-sm font-medium">{version.authorName}</div>
                          <div className="text-xs text-gray-500">
      ...
      {documents.length === 0 && !isCreating && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">No Collaborative Documents</h3>
          <p className="text-gray-600 mb-4">Create your first shared document to start collaborating with family</p>
          <Button onClick={createNewDocument} className="gap-2">
            <Plus className="h-4 w-4" />
