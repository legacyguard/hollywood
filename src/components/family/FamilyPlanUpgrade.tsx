...
interface FamilyPlanUpgradeProps {
  ...
  onPlanSelect: (planType: 'free' | 'family' | 'premium') => void;
...
export function FamilyPlanUpgrade({
  ...
  const [selectedPlan, setSelectedPlan] = useState<'free' | 'family' | 'premium'>(currentPlan);
  ...
  const renderFeatureList = (plan: FamilyPlan) => {
    const features = [
      { key: 'maxMembers', label: plan.maxMembers === -1 ? 'Unlimited family members' : `Up to ${plan.maxMembers} family members`, enabled: true },
  ...
  const PlanCard = ({ planKey, plan }: { planKey: 'free' | 'family' | 'premium', plan: FamilyPlan }) => {
      ...
      >
        <Card
          className={`relative cursor-pointer transition-all duration-200 ${
            isSelected ? planColors[planKey] : 'border-gray-200 hover:border-gray-300'
        ...
        >
          ...
          <CardHeader className="text-center pb-4">
            <div className="flex items-center justify-center mb-2">
              {planKey === 'premium' && <Crown className="h-6 w-6 text-purple-600 mr-2" />}
              {planKey === 'family' && <Users className="h-6 w-6 text-blue-600 mr-2" />}
              {planKey === 'free' && <Heart className="h-6 w-6 text-gray-600 mr-2" />}
            ...
            {plan.pricing.monthly > 0 ? (
              <div className="space-y-1">
                <div className="text-3xl font-bold">
                  ${billingCycle === 'yearly' ? plan.pricing.yearly / 12 : plan.pricing.monthly}
                  <span className="text-sm font-normal text-muted-foreground">/month</span>
                </div>
                {billingCycle === 'yearly' && (
                  <div className="text-sm text-green-600">
                    Save {getSavings(planKey as 'family' | 'premium')}% with yearly billing
                  </div>
                )}
              </div>
            ) : (
            ...
            {plan.maxMembers !== -1 && (
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className={limitProgress >= 90 ? 'text-red-600' : 'text-muted-foreground'}>
              ...
              <div className="pt-4 border-t">
                <div className="text-center space-y-2">
                  ...
                  <div className="text-lg font-bold">
                    ${billingCycle === 'yearly' ? plan.pricing.yearly : plan.pricing.monthly * 12}
            ...
            <CardContent className="p-6">
              <div className="text-center space-y-4">
                <div className="flex items-center justify-center gap-2">
                  ...
                  <h3 className="text-xl font-semibold">Family Protection Benefits</h3>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-3 text-sm">
                  <div className="text-center space-y-2">
                    ...
                    <div className="text-muted-foreground">
                      Multiple family members create redundancy and shared responsibility
                    </div>
                  </div>
                  ...
                  <div className="text-center space-y-2">
                    ...
                    <div className="text-muted-foreground">
                      Designated emergency contacts can access critical information when needed
                    </div>
                  </div>
                  ...
                  <div className="text-center space-y-2">
                    ...
                    <div className="text-muted-foreground">
                      Know your family is protected and informed about important matters
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="flex items-center gap-4">
              {selectedPlan !== currentPlan && (
                <div className="text-sm text-muted-foreground">
                  Selected: <span className="font-medium capitalize">{selectedPlan} Plan</span>
                </div>
              )}
              ...
              >
                {selectedPlan === currentPlan ? (
                  ...
                  <>
                    <Crown className="h-4 w-4" />
                    {FAMILY_PLANS[selectedPlan].trialDays ? 'Start Free Trial' : 'Upgrade Now'}
                  </>
                )}
              </Button>
            </div>
