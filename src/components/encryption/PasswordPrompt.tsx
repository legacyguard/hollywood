...
export function PasswordPrompt() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (success) {
      ...
      if (attempts >= 2) {
        setError('Too many failed attempts. Check your password or contact support.');
      } else {
  ...
  return (
    <Dialog open={passwordPromptVisible} onOpenChange={(open) => !open && hidePasswordPrompt()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={isLoading || !password}>
              {isLoading ? 'Unlocking...' : needsMigration ? 'Migrate & Unlock' : 'Unlock'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
