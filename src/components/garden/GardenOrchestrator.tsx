...
interface GardenView {
  id: string;
  name: string;
  component: 'overview' | 'tree' | 'seed' | 'detailed';
...
export const GardenOrchestrator: React.FC<GardenOrchestratorProps> = ({
  ...
  useEffect(() => {
    if (currentView === 'auto' || defaultView === 'auto') {
      ...
      if (effectiveMode === 'empathetic') {
        selectedView = progress && progress.overallProgress > 30 ? 'detailed' : 'overview';
      } else if (effectiveMode === 'pragmatic') {
        selectedView = progress && progress.overallProgress > 0 ? 'tree' : 'seed';
      } else {
  ...
  const handleViewChange = useCallback(async (viewId: string) => {
    ...
    const transitionDelay = effectiveMode === 'empathetic' ? 300 :
                           effectiveMode === 'pragmatic' ? 100 : 200;
  ...
  useEffect(() => {
    ...
    if (enableCelebrations && progress && progress.completedMilestones > 0) {
      ...
      if (latestMilestone !== lastCelebration && progress.completedMilestones > 0) {
    ...
    return (
      ...
      >
        <div className="flex items-center gap-1">
          {gardenViews
            .filter(view => variant === 'premium' || view.personalityMatch.includes(effectiveMode))
            .map(view => {
          ...
          >
            {showControls_ ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
          </motion.button>
        </div>
      </motion.div>
    );
    ...
    >
      ...
      {variant !== 'minimal' && (
        ...
        >
          ...
          {effectiveMode === 'adaptive' && <Target className="w-3 h-3 text-purple-500" />}
