...
export function TimeCapsuleWizard({ isOpen, onClose, guardians, onCapsuleCreated }: TimeCapsuleWizardProps) {
  ...
  const isStepValid = (step: number): boolean => {
    switch (step) {
      case 1:
        return formData.recipient !== null && formData.recipient.name.trim() !== '' && formData.recipient.email.trim() !== '';
      case 2:
        return formData.deliveryCondition === 'ON_DEATH' || (formData.deliveryCondition === 'ON_DATE' && formData.deliveryDate !== undefined);
      case 3:
        return formData.recording !== undefined && formData.messageTitle.trim() !== '';
      case 4:
    ...
    try {
      ...
      const { data: uploadData, error: uploadError } = await supabase.storage
        ...
        .upload(filename, formData.recording.blob, {
          contentType: formData.recording.fileType === 'video' ? 'video/webm' : 'audio/ogg',
          cacheControl: '3600'
        });
      ...
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-hidden">
        ...
        <div className="space-y-4">
          ...
          <div className="text-center">
            <h3 className="text-lg font-semibold">{WIZARD_STEPS[currentStep - 1].title}</h3>
          ...
          <FadeIn key={currentStep} duration={0.3}>
            ...
            {currentStep === 2 && (
              <DeliveryStep
                ...
                onDeliveryConditionChange={(deliveryCondition) => updateFormData({ deliveryCondition })}
                onDeliveryDateChange={(deliveryDate) => updateFormData({ deliveryDate })}
              />
            )}
            ...
            {currentStep === 3 && (
              <RecordingStep
                ...
                onMessageTitleChange={(messageTitle) => updateFormData({ messageTitle })}
                onMessagePreviewChange={(messagePreview) => updateFormData({ messagePreview })}
