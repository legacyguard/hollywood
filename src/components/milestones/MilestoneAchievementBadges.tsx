...
export function MilestoneAchievementBadges({
  ...
  const getMilestoneIcon = (type: LegacyMilestone['type']) => {
    switch (type) {
      case 'first_document': return FileCheck;
      case 'family_protected': return Users;
      case 'will_complete': return Shield;
      case 'trust_score_milestone': return TrendingUp;
      case 'protection_level': return Crown;
      case 'time_milestone': return Calendar;
      case 'document_milestone': return FileCheck;
      default: return Sparkles;
    }
  };

  const getBadgeGradient = (color: LegacyMilestone['badge_data']['color'], achieved: boolean) => {
  ...
  const getRarityEffects = (rarity: LegacyMilestone['badge_data']['rarity']) => {
    switch (rarity) {
      case 'legendary':
        return {
  ...
  const prepareBadgeData = (): BadgeDisplayData[] => {
    const allMilestones = [
      ...
      ...(showUpcoming ? upcomingMilestones.map(m => ({ milestone: m, achieved: false })) : [])
    ];
            ...
            >
              ...
              >
                ...
                {data.achieved && rarityEffects.sparkleCount > 0 && (
                  <>
                    {Array.from({ length: rarityEffects.sparkleCount }).map((_, i) => (
                      <motion.div
                        key={i}
              ...
              <div className="absolute -bottom-1 left-1/2 transform -translate-x-1/2">
                <Badge
                  ...
                  className={cn(
                    ...
                    data.rarity === 'epic' && 'border-purple-400 text-purple-600 bg-purple-50',
                    data.rarity === 'rare' && 'border-blue-400 text-blue-600 bg-blue-50',
                    data.rarity === 'common' && 'border-gray-400 text-gray-600 bg-gray-50',
              ...
              {data.achieved && (
                <div className="absolute -bottom-6 left-1/2 transform -translate-x-1/2 text-xs text-gray-500 whitespace-nowrap">
                  {new Date(data.milestone.achievement_date).toLocaleDateString()}
                </div>
              )}
            </motion.div>
          </TooltipTrigger>
          ...
          <TooltipContent side="bottom" className="max-w-64">
            <div className="p-2">
              <div className="flex items-center gap-2 mb-1">
                <h4 className="font-semibold text-sm">{data.milestone.title}</h4>
                {!data.achieved && <Lock className="h-3 w-3 text-gray-500" />}
              </div>
              <p className="text-xs text-gray-600 mb-2">{data.milestone.description}</p>
              {data.achieved && (
  ...
  if (variant === 'showcase') {
    const featuredBadges = badgeData.filter(d => d.achieved && d.rarity !== 'common').slice(0, 3);
        ...
        <CardContent>
          {featuredBadges.length > 0 ? (
              ...
              {featuredBadges.map((data, index) => (
                <div key={data.milestone.id} className="text-center space-y-2">
                  ...
                  <div className="space-y-1">
                    <p className="text-sm font-medium">{data.milestone.title}</p>
            ...
            <div className="text-center py-6 text-gray-500">
              ...
              <p className="text-sm">Complete actions to earn your first badges!</p>
            </div>
          )}
        </CardContent>
      </Card>
        ...
        <div className="mt-6 p-4 bg-gradient-to-r from-blue-50 to-green-50 rounded-lg">
          ...
          <p className="text-sm text-blue-700">
            ...
            <span className="font-semibold">
              {Math.round(achievedMilestones.reduce((sum, m) => sum + m.family_benefit_score, 0) / achievedMilestones.length)}
            </span>{' '}
            points of protection for your family. Every badge represents a step toward their security and peace of mind.
          </p>
        </div>
      )}
    </div>
