...
import { TrendingUp, Users, Shield, Clock, Sparkles, ChevronRight } from 'lucide-react';
...
export function LegacyProgressVisualization({
  ...
  const [gardenStage, setGardenStage] = useState<'seed' | 'sprout' | 'growing' | 'blooming' | 'flourishing'>('seed');
  ...
  const conditions: MilestoneTriggerConditions = {
    ...
    daysSinceFirstDocument: documents.length > 0 ?
      Math.floor((Date.now() - new Date(documents[0]?.created_at || Date.now()).getTime()) / (1000 * 60 * 60 * 24)) : 0,
  ...
  const protectionDays = calculateFamilyProtectionDays(documents[0]?.created_at);
  const nextSuggestion = getNextMilestoneSuggestion(conditions, achievedMilestones);
  ...
  const getProgressMessage = (score: number, familyCount: number) => {
    const familyTerm = familyCount > 0 ? `${familyCount} family members` : 'your loved ones';
  ...
  const getGardenVisualization = (stage: typeof gardenStage) => {
    const stages = {
      seed: { emoji: '🌰', color: 'text-brown-600', description: 'Seeds planted' },
      sprout: { emoji: '🌱', color: 'text-green-400', description: 'First growth' },
      growing: { emoji: '🌿', color: 'text-green-500', description: 'Growing strong' },
      blooming: { emoji: '🌸', color: 'text-pink-500', description: 'Beautiful blooms' },
      flourishing: { emoji: '🌺', color: 'text-purple-500', description: 'Full flourish' }
    };
  ...
  if (variant === 'header') {
    return (
      <div className={cn('flex items-center gap-4 p-3 bg-gradient-to-r from-blue-50 to-green-50 rounded-lg', className)}>
        <div className="flex items-center gap-2">
          <div className="text-2xl">{getGardenVisualization(gardenStage).emoji}</div>
          <div>
            <div className="font-medium text-sm">{progress.overall}% Protected</div>
            <div className="text-xs text-gray-600">{protectionDays} days of protection</div>
          </div>
        </div>
    ...
    <Card className={cn('overflow-hidden', className)}>
      <CardHeader className="pb-4">
        <div className="flex items-start justify-between">
          <div>
            <CardTitle className="text-lg font-bold">Your Legacy Garden</CardTitle>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4 text-center">
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-center gap-1">
              ...
              <span className="text-lg font-bold">{familyMembersCount + emergencyContactsCount}</span>
            </div>
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-sm font-bold text-blue-700">{trustScore}/100</span>
              </div>
            </div>
        ...
        {nextSuggestion && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <h4 className="font-medium text-yellow-800 mb-2">Continue Growing Your Garden</h4>
            <p className="text-sm text-yellow-700 mb-3">{nextSuggestion.suggestion}</p>
            <div className="flex items-center justify-between">
