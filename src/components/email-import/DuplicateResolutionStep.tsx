...
export function DuplicateResolutionStep({
  ...
  const [choices, setChoices] = useState<Record<string, DuplicateResolutionChoice>>({});
  ...
  const handleChoiceChange = (documentId: string, action: DuplicateResolutionChoice['action']) => {
    setChoices(prev => ({
      ...prev,
      [documentId]: {
        documentId,
        action,
        newFilename: action === 'rename' ? customFilenames[documentId] : undefined
      }
    }));
  };
  ...
  const getActionDescription = (action: DuplicateResolutionChoice['action']) => {
    switch (action) {
      case 'skip':
  ...
  const calculateSpaceSavings = () => {
    const skippedDuplicates = duplicates.filter(d => choices[d.document.id]?.action === 'skip');
    const totalSize = skippedDuplicates.reduce((sum, d) => sum + d.document.size, 0);
    return {
  ...
  return (
    ...
    >
      ...
      <div className="text-center">
        ...
        <h3 className="text-xl font-semibold mb-2">Duplicate Documents Detected</h3>
        <p className="text-muted-foreground">
          We found {duplicates.length} document{duplicates.length !== 1 ? 's' : ''} that might already exist in your vault.
      ...
      {spaceSavings.count > 0 && (
        <Alert className="border-green-200 bg-green-50">
          ...
          <AlertDescription className="text-green-700">
            By skipping {spaceSavings.count} duplicate{spaceSavings.count !== 1 ? 's' : ''},
            you'll save {spaceSavings.sizeFormatted} of storage space.
          </AlertDescription>
        </Alert>
      )}
            ...
            <Card key={duplicate.document.id} className="border-orange-200">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1 text-xs">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {new Date(duplicate.document.metadata.date).toLocaleDateString()}
                      </div>
                  ...
                  {/* Existing Document */}
                  <div className="space-y-2">
                    <h4 className="font-medium text-gray-900">Existing Document</h4>
                    <div className="space-y-1 text-xs">
                      <div className="flex items-center gap-2">
                        <HardDrive className="h-3 w-3" />
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {(['skip', 'replace', 'keep_both', 'rename'] as const).map((action) => (
                      <Button
                        key={action}
                        variant={choice?.action === action ? 'default' : 'outline'}
                        size="sm"
                        onClick={() => handleChoiceChange(duplicate.document.id, action)}
                ...
                {choice?.action === 'rename' && (
                  <div className="space-y-2">
                    <Label htmlFor={`filename-${duplicate.document.id}`} className="text-sm">
                      New filename:
                    </Label>
                    <Input
                      id={`filename-${duplicate.document.id}`}
                      value={customFilename}
                      onChange={(e) => handleFilenameChange(duplicate.document.id, e.target.value)}
                ...
                <Alert className="border-blue-200 bg-blue-50">
                  ...
                  <AlertDescription className="text-blue-800 text-sm">
                    <strong>Recommendation:</strong> {getActionLabel(duplicate.recommendation)} - {getActionDescription(duplicate.recommendation)}
                  </AlertDescription>
                </Alert>
              </CardContent>
            </Card>
      ...
      <div className="flex items-center justify-between">
        <div className="text-sm text-muted-foreground">
          {duplicates.length} duplicate{duplicates.length !== 1 ? 's' : ''} to resolve
        </div>
