    ...
    const metricData: PerformanceMetric[] = [
      {
        ...
        unit: 'ms',
        target: 2500,
        status: metrics.LCP ? (metrics.LCP <= 2500 ? 'good' : metrics.LCP <= 4000 ? 'needs-improvement' : 'poor') : 'good',
        description: 'Largest Contentful Paint - Time for the largest content element to become visible',
      },
      {
        name: 'FID',
        value: metrics.FID,
        unit: 'ms',
        target: 100,
        status: metrics.FID ? (metrics.FID <= 100 ? 'good' : metrics.FID <= 300 ? 'needs-improvement' : 'poor') : 'good',
        description: 'First Input Delay - Time from first interaction to response',
      },
      {
        name: 'CLS',
        value: metrics.CLS,
        unit: '',
        target: 0.1,
        status: metrics.CLS ? (metrics.CLS <= 0.1 ? 'good' : metrics.CLS <= 0.25 ? 'needs-improvement' : 'poor') : 'good',
      ...
      },
      {
        ...
        unit: 'ms',
        target: 1800,
        status: metrics.FCP ? (metrics.FCP <= 1800 ? 'good' : metrics.FCP <= 3000 ? 'needs-improvement' : 'poor') : 'good',
      ...
      },
      {
        ...
        unit: 'ms',
        target: 600,
        status: metrics.TTFB ? (metrics.TTFB <= 600 ? 'good' : metrics.TTFB <= 800 ? 'needs-improvement' : 'poor') : 'good',
                  ...
                  <ul className="space-y-2">
                    {insights.recommendations.map((recommendation, index) => (
                      <li key={index} className="flex items-start gap-2 text-sm">
                        <Icon name="arrow-right" className="h-4 w-4 text-primary mt-0.5 flex-shrink-0" />
