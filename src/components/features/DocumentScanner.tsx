        ...
        <div className="text-center space-y-4">
          <FadeIn duration={0.5}>
            ...
            <p className="text-muted-foreground">
              Upload a document image or PDF to extract text and automatically categorize it
            </p>
          </FadeIn>
          ...
          <Card className="p-6">
            <div className="space-y-3">
              ...
              <p className="text-sm text-muted-foreground">
                ...
                {processingProgress >= 60 && processingProgress < 90 && 'Analyzing and categorizing...'}
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      setConfig(prev => ({ ...prev, enableEntityExtraction: checked }))
                    }
                  />
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      setConfig(prev => ({ ...prev, enableDocumentClassification: checked }))
                    }
                  />
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      setConfig(prev => ({ ...prev, enableMetadataExtraction: checked }))
                    }
                  />
                </div>
              </div>
                ...
                <div>
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="accurate">Accurate (Better results)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
...
function DocumentResults({ document }: { document: ProcessedDocument }) {
  const [activeTab, setActiveTab] = useState<'overview' | 'text' | 'entities' | 'metadata'>('overview');
  ...
  return (
    <Card className="p-6">
      ...
      <div className="flex gap-1 mb-6 bg-muted/30 p-1 rounded-lg">
        {[
          { key: 'overview' as const, label: 'Overview', icon: 'info' as const },
          { key: 'text' as const, label: 'Extracted Text', icon: 'documents' as const },
          { key: 'entities' as const, label: 'Entities', icon: 'sparkles' as const },
          { key: 'metadata' as const, label: 'Metadata', icon: 'settings' as const }
        ].map(tab => (
          <button
            key={tab.key}
            ...
            <div>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span>{Math.round(document.ocrResult.confidence * 100)}%</span>
                </div>
                <div className="flex justify-between">
                  <span>Language:</span>
                  <span>{document.ocrResult.detectedLanguage.toUpperCase()}</span>
                </div>
                <div className="flex justify-between">
                  <span>Entities Found:</span>
                  <span>{document.ocrResult.metadata.extractedEntities.length}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            {document.ocrResult.metadata.extractedEntities.length > 0 ? (
              <div className="grid gap-3">
                {document.ocrResult.metadata.extractedEntities.map((entity, index) => (
