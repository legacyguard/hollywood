...
export default function EnhancedDocumentList({
  ...
  const [sortBy, setSortBy] = useState<'date' | 'importance' | 'name' | 'confidence'>('date');
  ...
  const getDocumentIcon = (doc: Document): string => {
    if (doc.processing_status === 'completed' && doc.ocr_text) return 'sparkles';
  ...
  const getStatusBadge = (doc: Document) => {
    if (doc.processing_status === 'completed' && doc.ocr_text) {
      return <Badge variant="secondary" className="text-xs">AI Processed</Badge>;
    }
    if (doc.processing_status === 'processing') {
      return <Badge variant="outline" className="text-xs">Processing...</Badge>;
    }
    if (doc.processing_status === 'failed') {
      return <Badge variant="destructive" className="text-xs">Processing Failed</Badge>;
    }
    return null;
  };
      ...
      <div onClick={onSelect} className="space-y-4">
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
              document.category === 'legal' ? 'bg-blue-500/10' :
              document.category === 'financial' ? 'bg-green-500/10' :
              document.category === 'medical' ? 'bg-red-500/10' :
              document.category === 'insurance' ? 'bg-purple-500/10' :
              'bg-primary/10'
            }`}>
              <Icon name={getDocumentIcon(document)} className={`w-5 h-5 ${
                document.category === 'legal' ? 'text-blue-600' :
                document.category === 'financial' ? 'text-green-600' :
                document.category === 'medical' ? 'text-red-600' :
                document.category === 'insurance' ? 'text-purple-600' :
                'text-primary'
              }`} />
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {!isExpired(document.expires_at) && isExpiringSoon(document.expires_at) && (
        ...
        <div className="space-y-2 text-xs text-muted-foreground">
          ...
          {document.expires_at && (
            <div className="flex justify-between">
              <span>Expires:</span>
              <span className={isExpired(document.expires_at) ? 'text-red-600' : ''}>
    ...
    <Card className="p-4 hover:shadow-md transition-all duration-200 cursor-pointer group">
      <div onClick={onSelect} className="flex items-center gap-4">
        <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${
          ...
          <Icon name={getDocumentIcon(document)} className={`w-6 h-6 ${
            document.category === 'legal' ? 'text-blue-600' :
            document.category === 'financial' ? 'text-green-600' :
            document.category === 'medical' ? 'text-red-600' :
            document.category === 'insurance' ? 'text-purple-600' :
