...
export default function EnhancedDocumentUploader({ 
  ...
  const handleDocumentProcessed = useCallback((doc: ProcessedDocument) => {
    ...
    setManualFormData(prev => ({
      ...
      description: doc.ocrResult.text.substring(0, 200) + (doc.ocrResult.text.length > 200 ? '...' : ''),
  ...
  const handleSaveDocument = useCallback(async () => {
    ...
    try {
      ...
      const documentData = {
        ...
        extracted_entities: processedDocument?.ocrResult.metadata.extractedEntities || null,
        classification_confidence: processedDocument?.classification.confidence || null,
              ...
              <div className="mt-6 p-4 bg-primary/5 border border-primary/20 rounded-lg">
                ...
                <div className="grid md:grid-cols-3 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground">
                      {Math.round(processedDocument.ocrResult.confidence * 100)}% accuracy
                    </p>
                  </div>
                  <div>
                    <span className="font-medium">Entities Found:</span>
                    <p className="text-muted-foreground">
                      {processedDocument.ocrResult.metadata.extractedEntities.length} items
                    </p>
                  </div>
                </div>
              </div>
