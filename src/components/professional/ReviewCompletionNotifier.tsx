...
export function ReviewCompletionNotifier({
  ...
  const getNotificationVariant = (): NotificationVariant => {
    if (result.overall_status === 'approved') {
      return {
        ...
        message: 'Your document has been professionally approved with high confidence.',
        icon: CheckCircle,
      ...
      return {
        ...
        message: 'Your document is solid with some helpful recommendations for improvement.',
        icon: FileText,
      ...
      return {
        ...
        message: 'Important issues were found that need attention for your family\'s protection.',
        icon: AlertTriangle,
      ...
      return {
        ...
        message: 'Critical issues were identified that could impact your family\'s security.',
        icon: AlertTriangle,
  ...
  const handleSubmitRating = async () => {
    ...
    try {
      ...
      toast({
        title: "Thank You!",
        description: "Your feedback helps us maintain high-quality professional services.",
      });
      setShowRating(false);
    } catch (_error) {
      toast({
    ...
    <AnimatePresence>
      ...
      >
        ...
        <Card className={cn(
          ...
          variant.type === 'success' && 'bg-gradient-to-br from-green-50 to-emerald-50',
          variant.type === 'info' && 'bg-gradient-to-br from-blue-50 to-indigo-50',
          variant.type === 'warning' && 'bg-gradient-to-br from-orange-50 to-amber-50'
        )}>
          {/* Header with gradient */}
          <div className={cn('bg-gradient-to-r text-white p-6 relative overflow-hidden', variant.gradient)}>
          ...
          <CardContent className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              >
                <div className={cn(
                  ...
                  result.overall_status === 'approved' ? 'text-green-600' :
                  result.overall_status.includes('approved') ? 'text-blue-600' : 'text-orange-600'
                )}>
            ...
            <div className="bg-white rounded-lg p-4 shadow-sm">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">{reviewer.professional_title}</p>
                  {reviewer.law_firm_name && (
                    <p className="text-sm text-gray-500">{reviewer.law_firm_name}</p>
                  )}
                </div>
            ...
            {/* Key Highlights */}
            {(result.recommendations.length > 0 || result.issues_found.length > 0) && (
              <div className="space-y-3">
                ...
                {result.recommendations.slice(0, 2).map((rec, index) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-green-700 leading-relaxed">{rec.description}</p>
                    </div>
                  </motion.div>
                ))}
                ...
                {result.issues_found.slice(0, 2).map((issue, index) => (
                  ...
                  >
                    ...
                    <div>
                      <p className="text-sm font-medium text-orange-900">{issue.title}</p>
                      <p className="text-xs text-orange-700 leading-relaxed">{issue.description}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-4">
              <Button
                ...
                className={cn('flex-1 bg-gradient-to-r text-white shadow-lg hover:shadow-xl transition-all', variant.gradient)}
              >
                {variant.actionText}
                  ...
                  <DialogContent className="max-w-md">
                    <div className="space-y-6">
                      <div className="text-center">
                        <h3 className="text-lg font-semibold mb-2">Rate This Review</h3>
                        <p className="text-sm text-gray-600">
                          How satisfied are you with {reviewer.full_name}'s review?
                        </p>
                      </div>
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-4 border border-blue-100">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="font-medium text-blue-900 mb-1">Family Protection Enhanced</h4>
                  <p className="text-sm text-blue-800 leading-relaxed">
                    This professional review strengthens your family's legal protection. The insights provided
                    help ensure your loved ones are better secured and your wishes are legally sound.
                  </p>
                </div>
              </div>
            </div>
  ...
  useEffect(() => {
    const getStatusEmoji = () => {
      switch (result.overall_status) {
        ...
        case 'approved_with_changes': return '✅';
        case 'requires_revision': return '⚠️';
        default: return '🚨';
      }
    };
    ...
    const getStatusMessage = () => {
      switch (result.overall_status) {
        case 'approved': return 'Your document has been approved!';
        case 'approved_with_changes': return 'Review complete with recommendations';
        case 'requires_revision': return 'Review complete - action required';
        default: return 'Review complete - critical issues found';
      }
    };
      ...
      description: (
        <div className="space-y-2">
          ...
          <p className="text-xs text-gray-600">
            Reviewed by {reviewer.full_name} • Trust Score: +{result.trust_score_impact}
          </p>
          <Button
            size="sm"
            onClick={onViewReport}
