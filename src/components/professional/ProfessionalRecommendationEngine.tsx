...
interface RecommendationCriteria {
  serviceType: 'review' | 'consultation' | 'ongoing';
  ...
  communicationPreference: 'phone' | 'video' | 'in_person' | 'any';
...
interface ProfessionalRecommendation {
  ...
  availability: 'immediate' | 'same_day' | 'this_week' | 'next_week';
  specialtyMatch: boolean;
  locationMatch: boolean;
  budgetFit: 'perfect' | 'good' | 'stretch';
  clientReviews: {
    rating: number;
    count: number;
    recentReviews: Array<{
...
interface ProfessionalRecommendationEngineProps {
  onSelectProfessional: (professional: ProfessionalReviewer, service?: string) => void;
...
const SAMPLE_PROFESSIONALS: ProfessionalRecommendation[] = [
  {
    ...
    clientReviews: {
      ...
      recentReviews: [
        {
          rating: 5,
          comment: 'Exceptional service and attention to detail. Made the estate planning process smooth and understandable.',
          date: '2024-01-10',
          verified: true
        },
        {
          rating: 5,
          comment: 'Sarah helped us navigate complex tax implications with our family trust. Highly recommended.',
          date: '2024-01-05',
          verified: true
        }
      ]
  ...
  },
  {
    ...
    clientReviews: {
      ...
      recentReviews: [
        {
          rating: 5,
          comment: 'Michael provided excellent guidance for our business succession plan. Very thorough and professional.',
          date: '2024-01-08',
          verified: true
        }
      ]
...
export function ProfessionalRecommendationEngine({
  ...
  const calculateMatchScore = (
    ...
    const profSpecs = professional.professional.specializations?.map(s => s.name.toLowerCase()) || [];
    const requiredSpecs = criteria.requiresSpecializations.map(s => s.toLowerCase());
    const specMatch = requiredSpecs.filter(spec =>
      profSpecs.some(profSpec => profSpec.includes(spec) || spec.includes(profSpec))
    ).length;
    ...
    else if (criteria.experienceLevel === 'senior' && expYears >= 10) score += 15;
    else if (criteria.experienceLevel === 'junior' && expYears < 10) score += 10;
    else if (criteria.experienceLevel === 'any') score += 10;

    // Budget fit
    const hourlyRate = professional.professional.hourly_rate || 0;
    if (hourlyRate >= criteria.budget.min && hourlyRate <= criteria.budget.max) {
      score += 15;
    ...
    if (criteria.urgency === 'high' && professional.availability === 'same_day') {
      score += 15;
    } else if (criteria.urgency === 'medium' && ['same_day', 'this_week'].includes(professional.availability)) {
      score += 10;
  ...
  const getBudgetFitColor = (fit: ProfessionalRecommendation['budgetFit']) => {
    switch (fit) {
      ...
      case 'stretch': return 'text-orange-600 bg-orange-100';
      default: return 'text-gray-600 bg-gray-100';
    }
  };

  const getAvailabilityColor = (availability: ProfessionalRecommendation['availability']) => {
    switch (availability) {
      case 'immediate': return 'text-green-600 bg-green-100';
      case 'same_day': return 'text-green-600 bg-green-100';
      case 'this_week': return 'text-blue-600 bg-blue-100';
      case 'next_week': return 'text-orange-600 bg-orange-100';
      default: return 'text-gray-600 bg-gray-100';
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="text-center space-y-4">
        ...
        <h2 className="text-3xl font-bold">Find Your Perfect Legal Professional</h2>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Our intelligent matching system analyzes your needs and connects you with the most suitable
          attorneys based on expertise, availability, and client success history.
        </p>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value: 'review' | 'consultation' | 'ongoing') =>
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="ongoing">Ongoing Representation</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value: 'any' | 'junior' | 'senior' | 'expert') =>
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    <div>
                      <Label className="text-base font-medium mb-3">Required Specializations</Label>
                      <div className="grid grid-cols-2 gap-2">
                        {SPECIALIZATIONS.map(spec => (
                          <Button
                            key={spec}
                            variant={criteria.requiresSpecializations.includes(spec) ? 'default' : 'outline'}
                            size="sm"
                            onClick={() => {
                              setCriteria(prev => ({
                                ...prev,
                                requiresSpecializations: prev.requiresSpecializations.includes(spec)
                                  ? prev.requiresSpecializations.filter(s => s !== spec)
                    ...
                    <div>
                      <Label className="text-base font-medium mb-3">Document Types</Label>
                      <div className="grid grid-cols-2 gap-2">
                        {DOCUMENT_TYPES.slice(0, 4).map(type => (
                          <Button
                            key={type}
                            variant={criteria.documentTypes.includes(type) ? 'default' : 'outline'}
                            size="sm"
                            onClick={() => {
                              setCriteria(prev => ({
                                ...prev,
                  ...
                  <div className="space-y-4">
                    <div>
                      <Label className="text-base font-medium mb-3">Budget Range</Label>
                    ...
                    <div>
                      <Label className="text-base font-medium mb-3">Family Context</Label>
                      <div className="space-y-3">
                        {Object.entries({
                          ...
                          <div key={key} className="flex items-center justify-between">
                            ...
                            <Switch
                              checked={criteria.familyContext[key as keyof typeof criteria.familyContext]}
                              onCheckedChange={(checked) =>
                                setCriteria(prev => ({
                                  ...prev,
                                  familyContext: { ...prev.familyContext, [key]: checked }
                                }))
                              }
                            />
                          </div>
      ...
      {isLoading && (
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-lg font-semibold mb-2">Finding Perfect Matches</h3>
            <p className="text-muted-foreground">
              Analyzing hundreds of professionals to find your ideal legal counsel...
            </p>
                ...
                >
                  <Card className="hover:shadow-lg transition-all duration-300 border-2 hover:border-blue-200">
                    <CardContent className="p-6">
                          ...
                          <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1">
                              <div className="flex items-start justify-between mb-2">
                                <div>
                                  <h4 className="text-xl font-semibold">{professional.full_name}</h4>
                                  <p className="text-muted-foreground">{professional.professional_title}</p>
                                  {professional.law_firm_name && (
                                    <p className="text-sm text-muted-foreground">{professional.law_firm_name}</p>
                                  )}
                                </div>
                              ...
                              <div className="flex items-center gap-4 text-sm text-muted-foreground mb-3">
                                ...
                                <span className="flex items-center gap-1">
                                  ...
                                  {recommendation.clientReviews.rating} ({recommendation.clientReviews.count} reviews)
                                </span>
                                <span className="flex items-center gap-1">
                                  <MapPin className="h-4 w-4" />
                                  {professional.licensed_states?.[0]}
                                  {professional.licensed_states && professional.licensed_states.length > 1 &&
                        ...
                        <div className="space-y-4">
                          <div className="grid grid-cols-2 gap-2 text-center">
                            <div className="p-3 bg-gray-50 rounded-lg">
                              ...
                              <Badge className={cn('text-xs mt-1', getAvailabilityColor(recommendation.availability))}>
                                {recommendation.availability.replace('_', ' ')}
                              </Badge>
                            </div>
                            <div className="p-3 bg-gray-50 rounded-lg">
                              <p className="text-xs text-muted-foreground">Budget Fit</p>
                              <Badge className={cn('text-xs mt-1', getBudgetFitColor(recommendation.budgetFit))}>
                          ...
                          <div className="space-y-3">
                            <Label className="font-medium">Suggested Services:</Label>
                          ...
                          <div className="space-y-2">
                            <Button
                              onClick={() => onSelectProfessional(professional, 'consultation')}
                              className="w-full"
                            >
                      ...
                      {recommendation.clientReviews.recentReviews.length > 0 && (
                        <div className="mt-6 pt-6 border-t">
                          <Label className="font-medium mb-3 block">Recent Client Reviews:</Label>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {recommendation.clientReviews.recentReviews.map((review, idx) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              All attorneys are licensed, verified, and maintain professional liability insurance
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              Advanced algorithms ensure you're connected with the most suitable legal counsel
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              100% satisfaction guarantee with transparent pricing and clear expectations
            </p>
          </div>
        </div>
      </div>
