...
interface ReviewRequestWorkflowProps {
  ...
  onRequestSubmitted: (request: Omit<ReviewRequest, 'id' | 'user_id' | 'created_at' | 'updated_at'>) => void;
  onCancel?: () => void;
  className?: string;
}
          ...
          >
            <div>
              ...
              <div className="space-y-4">
                {(Object.entries(REVIEW_TYPES) as Array<[keyof typeof REVIEW_TYPES, typeof REVIEW_TYPES[keyof typeof REVIEW_TYPES]]>).map(([key, type]) => (
                  <Card
                    key={key}
                    className={cn(
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div className="text-right">
                          <div className="font-semibold text-green-600">{type.estimatedCost}</div>
            ...
            <div>
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="low">Low Priority - Standard timeline</SelectItem>
                  <SelectItem value="medium">Medium Priority - Slightly expedited</SelectItem>
                  <SelectItem value="high">High Priority - Expedited (+25% fee)</SelectItem>
                  <SelectItem value="urgent">Urgent - Rush service (+50% fee)</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </motion.div>
              ...
              <div className="space-y-4">
                      ...
                      <div key={spec} className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => {
                            if (checked) {
                              updateForm({
                                required_specializations: [...form.required_specializations, spec]
                              });
                            } else {
                              updateForm({
                        ...
                        />
                        <Label htmlFor={spec} className="font-normal capitalize">
                          {spec.replace('_', ' ')}
                        </Label>
                      </div>
                    ))}
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => updateForm({ budget_max: e.target.value ? parseInt(e.target.value) : undefined })}
                    placeholder="e.g., 500"
                    className="mt-1"
                  />
              ...
              <div className="space-y-3">
                {getRecommendedReviewers().map(reviewer => (
                  <Card
                    ...
                    onClick={() => updateForm({ preferred_reviewer_id: reviewer.id })}
                  >
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between">
                        <div>
                          <h5 className="font-semibold">{reviewer.full_name}</h5>
                        ...
                        <div className="text-right">
                          ...
                          {reviewer.hourly_rate && (
                            <p className="text-sm text-green-600">${reviewer.hourly_rate}/hr</p>
                          )}
                        </div>
                      </div>
                ...
                <Card
                  ...
                  onClick={() => updateForm({ preferred_reviewer_id: undefined })}
                >
                  <CardContent className="p-4 text-center">
                    <Users className="h-8 w-8 text-gray-400 mx-auto mb-2" />
                    <p className="text-sm font-medium">Auto-assign best match</p>
                    <p className="text-xs text-gray-600">Let us find the perfect attorney for your needs</p>
                  </CardContent>
                </Card>
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-6">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium">{REVIEW_TYPES[form.review_type].name}</span>
                </div>
                <div className="flex items-center justify-between">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium text-green-600">${calculateEstimatedCost()}</span>
                </div>
                {form.deadline && (
                  <div className="flex items-center justify-between">
                    <span className="text-gray-600">Deadline:</span>
                    <span className="font-medium">{new Date(form.deadline).toLocaleDateString()}</span>
                  </div>
                )}
                <div className="flex items-center justify-between">
                  <span className="text-gray-600">Turnaround Time:</span>
                  <span className="font-medium">{REVIEW_TYPES[form.review_type].turnaroundTime}</span>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <ol className="text-sm text-blue-700 space-y-1 list-decimal list-inside">
                    <li>We'll match you with a qualified attorney within 2 hours</li>
                    <li>You'll receive an email with attorney details and confirmation</li>
                    <li>The attorney will begin their review within 24 hours</li>
                    <li>You'll get progress updates throughout the review process</li>
                  </ol>
                </div>
              </div>
            </div>
    ...
    <div className={cn('max-w-2xl mx-auto', className)}>
      ...
      <div className="text-center mb-8">
        <div className="flex items-center justify-center gap-3 mb-4">
          ...
          <h1 className="text-3xl font-bold text-gray-900">Request Professional Review</h1>
        </div>
        <p className="text-gray-600">
          Get your {documentType.toLowerCase()} professionally reviewed by licensed attorneys
        </p>
      </div>
