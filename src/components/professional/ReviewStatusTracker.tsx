...
import type { DocumentReview, ProfessionalReviewer, ReviewResult } from '@/types/professional';
    ...
    switch (review.status) {
      case 'requested':
        return {
          ...
          message: 'We\'re matching you with the perfect attorney for your document review.',
      ...
      case 'completed':
        return {
          ...
          message: 'Your professional review is complete and results are available.',
          action: 'View your detailed report and recommendations'
        };
      case 'rejected':
        return {
          title: 'Review Declined',
          message: 'This review was declined. We\'re finding an alternative attorney.',
  ...
  const getReviewSteps = (): ReviewStep[] => {
    const steps: ReviewStep[] = [
      ...
      },
      {
        id: 'assigned',
        title: 'Attorney Assigned',
        description: reviewer ? `Assigned to ${reviewer.full_name}` : 'Waiting for assignment',
      ...
      },
      {
        ...
        status: review.status === 'completed' ? 'completed'
              : review.status === 'in_progress' ? 'current'
              : 'pending',
        timestamp: review.status === 'in_progress' ? new Date().toISOString() : undefined,
        icon: Scale
      },
      {
        id: 'completed',
      ...
      <Card>
        <CardContent className="p-6">
          <div className="flex items-start justify-between mb-6">
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-2">Professional Review Status</h2>
              <div className="flex items-center gap-3">
                <Badge className={cn('text-sm', getStatusColor(review.status))}>
                  {review.status.replace('_', ' ').toUpperCase()}
                </Badge>
                <span className="text-gray-500 text-sm">Requested {timeElapsed}</span>
              </div>
            </div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-semibold text-blue-900 mb-1">{statusInfo.title}</h3>
                <p className="text-blue-800 text-sm mb-2 leading-relaxed">{statusInfo.message}</p>
                {statusInfo.action && (
                  <p className="text-blue-600 text-xs font-medium">{statusInfo.action}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <CardContent>
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-gray-600 mb-2">{reviewer.professional_title}</p>
                {reviewer.law_firm_name && (
                  <p className="text-gray-500 text-sm mb-2">{reviewer.law_firm_name}</p>
                )}
                ...
                <div className="flex items-center gap-4 mb-3">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-sm">{reviewer.experience_years}+ years</span>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="text-center p-4 bg-blue-50 rounded-lg">
                  <div className="text-2xl font-bold text-blue-700 mb-1">
                    {reviewResult.overall_status.replace('_', ' ').toUpperCase()}
                  </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600">
                Our support team is here to assist you throughout the review process.
              </p>
            </div>
