...
export function AttorneyDashboard({
  ...
  useEffect(() => {
    ...
    const totalEarnings = completedReviews.reduce(
      (sum, review) => sum + (review.review_fee || 0), 0
    );
  ...
  const getReviewTypeLabel = (type: DocumentReview['review_type']) => {
    switch (type) {
      case 'basic': return 'Basic Review';
      case 'comprehensive': return 'Comprehensive Review';
      case 'certified': return 'Certified Review';
    ...
    <div className={cn('space-y-6', className)}>
      ...
      <Tabs defaultValue="pending" className="space-y-6">
        <TabsList className="grid w-full grid-cols-3">
          <TabsTrigger value="pending">Pending Reviews ({stats.pendingReviews})</TabsTrigger>
          <TabsTrigger value="completed">Completed ({stats.totalReviews})</TabsTrigger>
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1 space-y-3">
                          <div className="flex items-center gap-3">
                            ...
                            <Badge className={cn('text-xs', getPriorityColor(review.priority))}>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600">
                            <div className="flex items-center gap-2">
                              ...
                              <span>Requested {formatTimeAgo(review.requested_at)}</span>
                            </div>
                            {review.due_date && (
                              <div className="flex items-center gap-2">
              ...
              <div className="space-y-4">
                {completedReviews.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">No completed reviews yet</h3>
                    <p className="text-gray-600">Your completed reviews will appear here</p>
                  </div>
                ) : (
                  completedReviews.map((review) => (
                    <div key={review.id} className="flex items-center justify-between p-4 border rounded-lg">
                      <div>
                        <h4 className="font-medium">{getReviewTypeLabel(review.review_type)}</h4>
                        <p className="text-sm text-gray-600">
                          Completed {review.completed_at ? formatTimeAgo(review.completed_at) : 'Recently'}
                        </p>
                      </div>
                ...
                <div className="space-y-4">
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm font-medium">{stats.responseTime}h avg</span>
                    </div>
