...
export interface LegacyMilestone {
  id: string;
  type: 'first_document' | 'family_protected' | 'will_complete' | 'trust_score_milestone' | 'protection_level' | 'time_milestone' | 'document_milestone';
  title: string;
  description: string;
  ...
  celebration_data: {
    ...
    message: string;
    emoji: string;
    animation?: 'confetti' | 'firefly_trail' | 'garden_bloom' | 'certificate_glow';
  };
  badge_data: {
    icon: string;
    color: 'gold' | 'silver' | 'bronze' | 'emerald' | 'blue';
...
export function checkMilestoneAchievements(
  ...
  const conditions = buildTriggerConditions(documents, willData, familyMembersCount, emergencyContactsCount, trustScore);
  const newMilestones: LegacyMilestone[] = [];
  ...
  potentialMilestones.forEach(milestone => {
    if (!existingMilestoneIds.has(milestone.id) && isMilestoneAchieved(milestone, conditions)) {
...
function buildTriggerConditions(
  ...
  const firstDocDate = documents.length > 0
    ? Math.min(...documents.map(d => new Date(d.created_at).getTime()))
    : Date.now();
  const daysSinceFirst = Math.floor((Date.now() - firstDocDate) / (1000 * 60 * 60 * 24));
 ...
 */
function getPotentialMilestones(conditions: MilestoneTriggerConditions): LegacyMilestone[] {
  ...
  return [
    // First Document Milestone
    {
      ...
      description: 'You\'ve started building your family\'s foundation of security',
      achievement_date: new Date().toISOString(),
      family_impact_message: generateFamilyImpactStatement('first_document', familyContext).primary_message,
      celebration_data: {
    ...
    // Family Protection Started
    {
      ...
      description: 'Your first family member or emergency contact has access to your protection',
      achievement_date: new Date().toISOString(),
      ...
      celebration_data: {
        ...
        message: 'Someone trusted can now help your family when they need it most',
        emoji: '🛡️',
    ...
    // Protection Foundation (5+ documents)
    {
      ...
      description: 'Five essential documents secured - your family\'s safety net is taking shape',
      achievement_date: new Date().toISOString(),
      family_impact_message: 'Your family now has access to multiple important documents in any emergency',
      celebration_data: {
        title: 'Strong Foundation Built! 🏗️',
        message: 'Your family\'s protection foundation is solid with 5+ secured documents',
        emoji: '📁',
        animation: 'confetti'
      },
      badge_data: {
    ...
    // Will Completed
    {
      ...
      description: 'Your wishes are crystal clear - your family knows exactly what you want',
      achievement_date: new Date().toISOString(),
      family_impact_message: generateFamilyImpactStatement('will_completed', familyContext).primary_message,
      celebration_data: {
        title: 'Your Legacy is Clear! 📋',
        message: 'Your family won\'t have to guess your wishes - they\'ll know for certain',
        emoji: '⚖️',
    ...
    // Premium Protection Level
    {
      ...
      description: 'Your family enjoys top-tier security and comprehensive protection',
      achievement_date: new Date().toISOString(),
    ...
    // Trust Score Milestones
    {
      ...
      description: 'Achieved 75+ trust score - your family has strong protection in place',
      achievement_date: new Date().toISOString(),
      family_impact_message: 'Your family protection system is now rated as strong and reliable',
      celebration_data: {
    ...
    // Time-based milestone
    {
      ...
      description: 'Your family has been protected for 30 days - that\'s peace of mind growing stronger',
      achievement_date: new Date().toISOString(),
      family_impact_message: 'Your family has enjoyed 30 days of security and protection',
      celebration_data: {
        title: '30 Days of Family Protection! 📅',
        message: 'A full month of security - your preparation is paying dividends',
        emoji: '🌱',
        animation: 'garden_bloom'
      },
      badge_data: {
    ...
    // Comprehensive Protection
    {
      ...
      description: 'Maximum protection achieved - you are the ultimate guardian of your family\'s future',
      achievement_date: new Date().toISOString(),
      family_impact_message: 'Your family enjoys the highest possible level of protection and security',
      celebration_data: {
 ...
 */
function isMilestoneAchieved(milestone: LegacyMilestone, conditions: MilestoneTriggerConditions): boolean {
  switch (milestone.id) {
    ...
    case 'family_circle_started':
      return conditions.documentsCount >= 1 &&
             (conditions.familyMembersCount > 0 || conditions.emergencyContactsCount > 0);
    ...
    case 'premium_protection':
      return conditions.protectionLevel === 'premium' || conditions.protectionLevel === 'comprehensive';
    ...
    case 'comprehensive_guardian':
      return conditions.protectionLevel === 'comprehensive' &&
 ...
 */
export function calculateFamilyProtectionDays(firstDocumentDate?: string): number {
...
export function getNextMilestoneSuggestion(
...
): { milestone: string; suggestion: string; timeEstimate: string; impact: string } | null {

  if (!achievedMilestones.includes('first_document_uploaded') && conditions.documentsCount === 0) {
    return {
  ...
  if (!achievedMilestones.includes('family_circle_started') &&
      (conditions.familyMembersCount === 0 && conditions.emergencyContactsCount === 0)) {
    return {
  ...
  if (!achievedMilestones.includes('will_wisdom') && !conditions.willCompleted) {
    return {
  ...
  if (!achievedMilestones.includes('protection_foundation') && conditions.documentsCount < 5) {
    return {
...
export function getMilestoneProgress(conditions: MilestoneTriggerConditions): {
  ...
  if (conditions.familyMembersCount > 0 || conditions.emergencyContactsCount > 0) completedWeight += 15;
  ...
  if (conditions.documentsCount === 0) {
    ...
    progressToNext = 0;
  } else if (conditions.familyMembersCount === 0 && conditions.emergencyContactsCount === 0) {
